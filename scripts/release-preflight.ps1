...
# href="/KKXM112/starry-panel/blob/task-not-running.md" —— 没有 ref、没有目录，是一个 404。
...
if ($relativeLinks.Count -gt 0) {
    ...
    Fail-Step "Release notes must use absolute URLs. Found relative link target(s): $samples`nA GitHub Release body has no source-file path context, so [x](../task-not-running.md) is rendered as href=`"/KKXM112/starry-panel/blob/task-not-running.md`" - no ref, no directory, a guaranteed 404 (verified against the v3.2.3 release body).`nWrite https://github.com/KKXM112/starry-panel/blob/main/docs/<file> instead. Historical notes are intentionally left alone."
}
...
if (($updateJson -notmatch [regex]::Escape('"version": "' + $tagVersion + '"')) `
    ...
    -or ($updateJson -notmatch [regex]::Escape("/releases/download/$tagVersion/starry-panel-magisk-$tagVersion.zip")) `
...
if (($updateJsonDebian -notmatch [regex]::Escape('"version": "' + $tagVersion + '"')) `
    ...
    -or ($updateJsonDebian -notmatch [regex]::Escape("/releases/download/$tagVersion/starry-panel-magisk-debian-$tagVersion.zip")) `
...
Assert-FileTextContains -Path $workflowPath -Text "DOCKER_IMAGE_REPOSITORY: KKXM112/starry-panel" -Description "official Docker image repository"
...
foreach ($compose in @(
    ...
    if ([regex]::Matches($composeText, [regex]::Escape('${STARRY_PANEL_IMAGE:-')).Count -ne 2) {
        Fail-Step "$($compose.Name) must use STARRY_PANEL_IMAGE for both image and IMAGE_NAME."
    }
