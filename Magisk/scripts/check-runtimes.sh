...
#
# 用法:
#   su -c "sh /data/adb/modules/starry-panel/scripts/check-runtimes.sh"
#
# 用途:
#   检查满天星面板依赖的常见脚本运行时是否可用，并给出建议。
#   调用方式可以是 adb shell，也可以是模块卡片上的「运行」按钮。
##########################################################################

PANEL_DIR=/data/adb/starry-panel
