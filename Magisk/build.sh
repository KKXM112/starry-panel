#!/usr/bin/env bash
##########################################################################
# 满天星面板 Magisk 模块打包脚本 (容器方案 v2.0.6+)
#
# 用法（版本号必填）:
#   bash Magisk/build.sh 3.3.1                # arm64 + alpine
#   bash Magisk/build.sh 3.3.1 all            # 同时打包 arm64 + amd64
#   bash Magisk/build.sh 3.3.1 arm64 debian   # Debian(glibc) flavor
#
# 自检（不构建、不打包、不需要 go）:
#   bash Magisk/build.sh --check-dist         # 只校验已存在的 web/dist 是不是发布版产物
#
# 产物:
#   alpine（默认）: dist/starry-panel-magisk-v<版本>.zip
...
#      npm run build:demo  -> 在线演示 Demo：浏览器内 mock 层 + --base=/starry-panel/
...
assert_release_dist() {
  ...
  local demo_hits
  demo_hits="$(grep -rl '__STARRY_DEMO_MOCK__' "$dist/assets" 2>/dev/null || true)"
  if [ -n "$demo_hits" ]; then
    error "web/dist 是 Demo 产物：命中 mock 哨兵 __STARRY_DEMO_MOCK__，绝不能打进模块 ZIP"
  ...
  # （例如有人手动跑 npx vite build --base=/starry-panel/ 做验证后忘了还原）。
...
build_backend() {
  ...
  (cd "$ROOT/server" && \
    ...
    go build -trimpath \
      -ldflags="-s -w -X starry-panel/handler.Version=${VERSION}" \
      -o "$STAGING/system/bin/starry-server-${suffix}" .)
  (cd "$ROOT/server" && \
    CGO_ENABLED=0 GOOS=linux GOARCH="${go_arch}" \
    go build -trimpath \
      -ldflags="-s -w -X starry-panel/handler.Version=${VERSION}" \
...
# 两个 flavor 共用同一个 module id（starry-panel），而 updateJson 只能填一个 zipUrl，
