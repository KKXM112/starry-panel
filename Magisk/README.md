...
通过 Magisk / KernelSU / APatch 在已 Root 的 Android 设备上运行满天星面板。开机自启，浏览器访问 `http://127.0.0.1:5700` 即可使用；后端绑定 `0.0.0.0`，局域网 / 内网穿透也能直连。

> 本模块无需 Docker、无需 Termux。安装阶段会下载一份 rootfs 到 `/data/starry`，在容器里用包管理器装好 Python / Node.js / git 等运行时后，把 `starry-server` 放进容器启动。运行期等同于"用 root 起了一个极小号 Linux 容器跑面板"。
...
| | `starry-panel-magisk-vX.Y.Z.zip` | `starry-panel-magisk-debian-vX.Y.Z.zip` |
|---|---|---|
...
> **Debian 用户请不要点模块卡片上的「更新」按钮**（点了会把你换成 Alpine 版），改为每次手动到 [Release 页](https://github.com/KKXM112/starry-panel/releases) 下载 `starry-panel-magisk-debian-vX.Y.Z.zip` 安装。数据同样会被保留。
...
和 Docker 版的 Nginx + Go 两层结构不同，Magisk 版只有一层：前端静态资源由 `starry-server` 在单端口（默认 `5700`）上直接托管。整个运行时放进容器（通过仓库自带的 `rurima` 进入），数据 / 日志 / 脚本都落在容器内路径。

```
/data/adb/modules/starry-panel/        ← 模块本体（随模块卸载清除）
├── module.prop
...
└── system/bin/
    ├── rurima                         ← 容器运行时（静态二进制）
    ├── starry-server                  ← 后端（每次开机同步进容器）
    └── ddp                            ← CLI（同上）

/data/starry/（或 /data/local/starry/）← 容器 rootfs，Alpine 占 300MB+ / Debian 占 600MB+
├── usr/local/bin/starry-server        ← 实际在跑的后端
├── usr/local/bin/ddp                  ← 容器内 CLI
├── app/web/                           ← 前端静态
└── app/Starry-Panel/                    ← 所有用户数据
    ├── config.yaml                    ← 每次启动由 service.sh 重新生成
    ├── starry.db                      ← SQLite 数据库
    ├── starry.log / service.log       ← 后端 / 容器启动脚本日志
    └── scripts/ logs/ backups/ deps/

/data/adb/starry-panel/                ← 宿主侧持久化目录（不随模块升级清除）
...
1. 下载 ZIP（或按[下面章节](#本地构建)自行构建）：
   - Alpine（默认，推荐）：`starry-panel-magisk-vX.Y.Z.zip`
   - Debian（需要 glibc 时）：`starry-panel-magisk-debian-vX.Y.Z.zip`
...
```
updateJson=https://github.com/KKXM112/starry-panel/releases/latest/download/update.json
```
...
1. 每次仓库推送新的 `vX.Y.Z` tag，工作流会自动：
   - 编译 arm64 静态后端
   - 打包 `starry-panel-magisk-vX.Y.Z.zip`（Alpine）与 `starry-panel-magisk-debian-vX.Y.Z.zip`（Debian）
   - 导出并上传 `starry-debian-rootfs-arm64.tar.gz`（Debian 版安装时下载的 rootfs）
...
4. 重启手机完成升级。升级流程内部：`customize.sh` 先把容器里的 `/app/Starry-Panel/` 整个备份到 `/data/adb/starry-panel/update-data-backup`，然后清掉旧 rootfs 重装容器，装完再把备份复原回去——数据库、脚本、日志、依赖全部保留；如果下载或安装中途失败，下次重试会优先沿用这份备份恢复
...
- 替换容器内的 `/usr/local/bin/starry-server`、`/usr/local/bin/ddp`
- 替换前端目录 `/app/web`
...
> 说明：需要管理器版本支持 `updateJson`（Magisk v24.0+、KernelSU、APatch 新版均支持）。如果你自己 fork 了本项目发版，请把 `module.prop` 里的 `KKXM112/starry-panel` 替换成自己的仓库路径即可；`customize.sh` 里 Debian rootfs 的下载地址同样写着这个仓库路径，也要一起改。
...
模块内置 `action.sh`。在 **Magisk v26+ / KernelSU / APatch** 的模块列表里，满天星面板条目右侧会出现「运行 / Action」按钮。
...
**停止状态跨重启保持**：开关文件在 `/data/adb/starry-panel/stopped`，重启手机后 `service.sh` 会同步完模块文件就直接退出，不会把面板拉起来。再点一次动作按钮即可启动。
...
```bash
# 宿主侧 —— 启动日志
su -c "tail -f /data/adb/starry-panel/service.log"

# 进入容器（获得完整 bash / apk 或 apt / python / node / git / ddp）
MODDIR=/data/adb/modules/starry-panel
ROOTFS=/data/starry                   # 少数设备在 /data/local/starry
...
> **`ddp` 必须在容器内执行**——数据库路径 `/app/Starry-Panel/starry.db` 只在容器内有意义，直接用宿主侧 `/data/adb/modules/starry-panel/system/bin/ddp` 会找不到数据库。
...
su -c "sh /data/adb/modules/starry-panel/action.sh"
```

> ⚠️ 别再用 `su -c "pkill -f starry-server; sh .../service.sh"`（v3.0.3 及更早的文档里是这么写的）。两个问题：
>
> 1. 自 v3.0.3 起 `service.sh` 会 fork 一个存活守护，`pkill` 掉的面板 60 秒内就会被它拉回来；
> 2. 执行这条命令的 `sh -c` 进程自己的 cmdline 里就含 `starry-server`，会被 `pkill -f` 命中，分号后半句多半根本没执行到（用户看到「面板过一会儿又回来了」其实是守护拉的）。
>
> `action.sh` 用的是完整路径 `/usr/local/bin/starry-server` + `kill <pid>`，并且会先写停止开关让守护自退，不存在这两个问题。
...
```bash
# 进容器
su -c "/data/adb/modules/starry-panel/system/bin/rurima ruri -p -N -S -A /data/starry /bin/bash"
...
模块版没有 Docker 那套 nginx 反代，前端 / 后端都由同一个 `starry-server` 二进制在 `PANEL_PORT` 上直接托管，绑定 `0.0.0.0`，本机 / 局域网 / 内网穿透都能直连。

> **不要手动改 `/data/adb/starry-panel/config.yaml`**——每次开机 `service.sh` 都会按 `ports.conf` 重新生成 config.yaml，手动改的内容会被覆盖掉。

**端口配置的唯一入口**是 `/data/adb/starry-panel/ports.conf`（首次安装模块时自动生成，内含注释）：
...
> Debian 容器与 Alpine 3.23 容器还会同步写一份 `/etc/ssh/sshd_config.d/00-starry.conf`
...
> 单独再跑一次 `service.sh` 是**无效的**——它检测到 `starry-server` 已在跑会直接跳过（避免重复拉起）。必须先让旧进程退出，新进程才会按新 `ports.conf` 重新生成 `config.yaml` 并绑定新端口。而「让旧进程退出」现在只能走 `action.sh`：直接 `pkill` 的话，存活守护 60 秒内就把它拉回来了。
...
| 写入位置 | ✅ | 三处：`/data/adb/modules/starry-panel/`（模块本体）、`/data/starry/` 或 `/data/local/starry/`（容器 rootfs + 所有用户数据，占大空间）、`/data/adb/starry-panel/`（端口配置 + 启动日志 + 停止开关） |
...
> **禁用 ≠ 停服**：在管理器里「禁用」模块只阻止下次开机加载，**不会 kill 当前的容器进程**（`starry-server` 是 `rurima` 启的独立进程树）。想立即停：**点模块卡片的「运行 / Action」按钮**（面板在跑时这一次点击就是停止），或在面板里点「设置 → 概览 → 停止面板服务」。
>
> ⚠️ `su -c "pkill -f starry-server"` 自 v3.0.3 起就**停不掉**了：`service.sh` fork 的存活守护每分钟探活一次，60 秒内就会把它拉回来（刚拉起过的话最坏要等 5 分钟，看起来像是随机复活）。
...
- `TERM` + `KILL` 掉仍在运行的 `starry-server` 进程
- 删除容器 rootfs `/data/starry` 和 `/data/local/starry`（数百 MB ~ 1 GB，**面板所有数据都在这里**；两个 flavor 路径相同）
- 删除宿主侧持久化目录 `/data/adb/starry-panel/`（端口配置 + 启动日志）
- 清掉历史版本可能留下的 `init.d` / `service.d` 脚本

模块本体 `/data/adb/modules/starry-panel/` 由 Magisk / KernelSU / APatch 框架负责清除。重启完成后设备上不会残留任何满天星面板相关文件。
...
```bash
su -c "touch /data/adb/starry-panel/.keep_on_uninstall"
```

标记文件本体会随 rootfs 保留下来（就在 `/data/adb/starry-panel/` 里）。后续想彻底删：

```bash
su -c "rm -rf /data/starry /data/local/starry /data/adb/starry-panel"
```
...
备份落在容器的 `/app/Starry-Panel/backups/`，对应宿主侧路径是 `/data/starry/app/Starry-Panel/backups/`（KernelSU 用 `/data/local/starry/...`）。先 `adb pull` 或者 MT 管理器拷到电脑，然后再卸载即可。
...
bash Magisk/build.sh 3.3.1

# 只打 amd64
bash Magisk/build.sh 3.3.1 amd64

# 同时打 arm64 + amd64
bash Magisk/build.sh 3.3.1 all

# Debian flavor（第 3 个参数；不传就是 alpine）
bash Magisk/build.sh 3.3.1 arm64 debian
```
...
| flavor | 产物 |
|--------|------|
| `alpine`（默认，不传第 3 个参数） | `dist/starry-panel-magisk-v<版本>.zip` |
| `debian` | `dist/starry-panel-magisk-debian-v<版本>.zip` |
...
> `releases/latest/download/starry-debian-rootfs-arm64.tar.gz` 这个固定跳转，
...
> `web/dist`，Demo 产物里有整套浏览器内 mock 层，还带 `/starry-panel/` 的 base 前缀。
...
两种情况都不会损坏已有数据：升级安装时用户数据在清 rootfs 之前就已经备份到 `/data/adb/starry-panel/update-data-backup/`，abort 后备份原样保留，下次安装会自动恢复。

**Q: 浏览器打不开 `http://127.0.0.1:5700`**

1. 先点模块卡片「运行」按钮，看"监听端口"一行有没有 `LISTEN`。
2. 看 `/data/adb/starry-panel/service.log`（宿主侧启动日志）有没有 "面板启动失败"。
3. 进容器看 `/app/Starry-Panel/starry.log`（容器内后端日志）是否 panic 或端口被占。
4. MIUI / OriginOS / ColorOS 等激进省电策略会冻结后台进程，把管理器 daemon（`magiskd` / `ksud`）加入电池白名单；或打开浏览器时勾选"允许后台"。

**Q: 改了 `ports.conf` 但端口没生效**

`service.sh` 检测到 `starry-server` 已在跑会直接跳过，光重跑 `service.sh` 是不行的。必须先让旧进程退出。
...
⚠️ 别用 `pkill -f starry-server`：存活守护 60 秒内就会把它拉回来，而且执行这条命令的 `sh -c` 自身就会被 `pkill -f` 命中。

**Q: 升级后旧数据会丢吗？**

不会。升级流程：`customize.sh` → 备份 `<rootfs>/app/Starry-Panel/` 到 `/data/adb/starry-panel/update-data-backup/` → 清旧 rootfs → 重装 rootfs + 重装依赖 → 把备份复原回去。若安装中途失败，保留下来的 `update-data-backup` 会在下次安装时优先恢复。`ports.conf` 在宿主侧的 `/data/adb/starry-panel/` 不受影响。

**Q: 能从 Alpine 版直接换成 Debian 版吗？**

可以，直接装另一个 ZIP 即可，走的是和升级完全一样的流程：旧数据先备份到 `/data/adb/starry-panel/update-data-backup/`，旧 rootfs 整个删掉，装完新 rootfs 再把数据复原回去。数据库、脚本、备份都保留。
...
对，禁用 = 下次开机 Magisk 不挂载模块，不等于 kill 进程。`starry-server` 是 `rurima` 启的独立进程树，和模块本身解耦。

立即停用：**点模块卡片的「运行 / Action」按钮**（面板在跑时这一次就是停止），或在面板里点「设置 → 概览 → 停止面板服务」。停止状态跨重启保持，再点一次按钮即可启动回来。

⚠️ `su -c "pkill -f starry-server"` 停不掉：`service.sh` fork 的存活守护 60 秒内就会重新拉起面板（刚拉起过则最坏 5 分钟）。
