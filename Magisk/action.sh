#!/system/bin/sh
##########################################################################
# 满天星面板 Magisk 模块 - 快捷操作脚本（停止 / 启动 toggle + 状态摘要）
#
...
rootfs=/data/starry
[ ! -d "$rootfs" ] && rootfs=/data/local/starry

SERVER_LOG="$rootfs/app/Starry-Panel/starry.log"
TAIL_LINES=60
...
#
# 匹配的是完整路径 /usr/local/bin/starry-server（不是裸 starry-server）：
...
panel_pids() {
  ...
  for proc_dir in /proc/[0-9]*; do
    ...
    case "$proc_cmdline" in
      /usr/local/bin/starry-server*) _pids="$_pids ${proc_dir#/proc/}" ;;
    esac
  done
  _pids=${_pids# }
  [ -n "$_pids" ] || return 1
...
if [ -n "$ACTION_PIDS" ]; then
  ...
  # 2. 先 TERM 后 KILL。用 kill + 已探到的 PID，不用 `pkill -f starry-server`：
  ...
  if [ -f "$MODDIR/service.sh" ]; then
    ...
    if [ -n "$ACTION_PIDS" ]; then
      ...
      ui_print "! 启动后没有探测到面板进程，请看下面的 service.log / starry.log"
    fi
  else
...
# ---- starry.log (容器内后端日志) ----------------------------------------
ui_print " "
ui_print "--- starry.log (最近 ${TAIL_LINES} 行) ---"
...
ui_print " "
ui_print " 不要再用 \"pkill -f starry-server\" 停面板："
