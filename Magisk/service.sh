#!/system/bin/sh
##########################################################################
# 满天星面板 Magisk 模块 - late_start service
#
# 进入容器（Alpine 或 Debian，取决于 $MODDIR/flavor）启动 starry-server，
# 端口可通过 ports.conf 配置。前端静态资源由 starry-server 直接托管，不依赖 nginx。
##########################################################################
...
rootfs=/data/starry
if [ ! -d "$rootfs" ]; then
  rootfs=/data/local/starry
fi

# 模块目录探测
MODDIR=${MODDIR:-/data/adb/modules/starry-panel}
[ ! -d "$MODDIR" ] && MODDIR=/data/adb/magisk/modules/starry-panel
[ ! -d "$MODDIR" ] && MODDIR=/sbin/.magisk/modules/starry-panel
...
PERSIST_DIR=/data/adb/starry-panel
...
#   2. 容器内也可读写（面板的 Android 运行时就往 /data/adb/starry-panel/bin 落 Python/Node），
#      所以面板进程自己也能写停止开关；
#   3. 绝不能放 $rootfs/app/Starry-Panel/ 下 —— 那里的 .updating 每次开机被无条件删除，
...
log "========================================="
log "满天星面板模块启动 (MODDIR=$MODDIR, rootfs=$rootfs, flavor=$FLAVOR, shell=$CTR_SHELL)"
...
# ---- 把模块里的前端和 starry-server 同步进容器 ---------------------------
# 这里【不能】无条件覆盖。面板支持在面板内在线升级（只换 starry-server / ddp / web），
...
mkdir -p $rootfs/app/web $rootfs/app/Starry-Panel $rootfs/usr/local/bin

# 清理残留的在线升级哨兵。开机意味着上一次升级窗口一定已经结束；
# 若升级途中掉电或重启，哨兵会永久留在盘上，让下面的存活守护再也不敢接管。
rm -f "$rootfs/app/Starry-Panel/.updating" 2>/dev/null
...
if file_needs_sync "$MODDIR/system/bin/starry-server" "$rootfs/usr/local/bin/starry-server"; then
  cp -f  $MODDIR/system/bin/starry-server $rootfs/usr/local/bin/starry-server 2>/dev/null
  log "已从模块同步 starry-server"
else
  log "容器内 starry-server 不早于模块内版本，保留容器内版本（面板在线升级的结果）"
fi
chmod 755 $rootfs/usr/local/bin/starry-server 2>/dev/null

# 恢复持久化的依赖目录（容器 overlayfs 重启后可能丢失写入层）
DEPS_PERSIST="$PERSIST_DIR/deps-snapshot"
if [ -d "$DEPS_PERSIST" ]; then
  mkdir -p $rootfs/app/Starry-Panel/deps
  cp -rf "$DEPS_PERSIST/." $rootfs/app/Starry-Panel/deps/ 2>/dev/null
  ...
  if [ ! -f "$DEPS_PERSIST/nodejs/.starry-node-abi" ]; then
    rm -f "$rootfs/app/Starry-Panel/deps/nodejs/.starry-node-abi" 2>/dev/null
  fi
...
# 下面的「拉起容器」之前。
#
# 放太靠前的后果：用户在停止状态下刷入新模块 zip 再重启，新的 starry-server / web
...
STARTUP=$rootfs/tmp/starry-startup.sh
...
export STARRY_DIR=/app/Starry-Panel
export LANG=C.UTF-8
export HOME=/root
export SHELL=/bin/bash
export STARRY_MAGISK_MODULE=1
# 模块外壳（本脚本 + customize.sh + action.sh + rootfs 结构）的版本号。
# 面板的在线升级只替换 starry-server / ddp / web，覆盖不到外壳。
#
...
export STARRY_MAGISK_SHELL_VERSION=5
export STARRY_ANDROID_RUNTIME_BIN_DIR=/data/adb/starry-panel/bin
export PATH=/data/adb/starry-panel/bin/python/bin:/data/adb/starry-panel/bin/node/bin:/data/adb/starry-panel/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/app
export NODE_PATH=/usr/local/lib/node_modules

mkdir -p $STARRY_DIR/scripts $STARRY_DIR/logs $STARRY_DIR/deps/nodejs $STARRY_DIR/deps/python $STARRY_DIR/backups
chmod 777 $STARRY_DIR
...
if [ -f $STARRY_DIR/service.log ]; then
  _svc_log_size=$(stat -c%s $STARRY_DIR/service.log 2>/dev/null || echo 0)
  if [ "${_svc_log_size:-0}" -gt 1048576 ]; then
    mv -f $STARRY_DIR/service.log $STARRY_DIR/service.log.old 2>/dev/null
  fi
fi
...
case "$PY_MINOR" in
  3.10|3.11|3.12)
    export STARRY_PYTHON_VERSION="$PY_MINOR"
    if [ ! -d "$STARRY_DIR/deps/python/$PY_MINOR" ]; then
      python3 -m venv "$STARRY_DIR/deps/python/$PY_MINOR" 2>/dev/null || true
    fi
    ;;
esac
...
cat > $STARRY_DIR/config.yaml << YAML
server:
  port: ${PANEL_PORT}
  mode: release
  web_dir: /app/web
...
database:
  path: /app/Starry-Panel/starry.db
...
data:
  dir: /app/Starry-Panel
  scripts_dir: /app/Starry-Panel/scripts
  log_dir: /app/Starry-Panel/logs
...
if [ -n "${EXTRA_CORS_ORIGINS}" ]; then
  echo "${EXTRA_CORS_ORIGINS}" | tr ',;' '\n' | while IFS= read -r origin; do
    ...
    [ -z "$origin" ] && continue
    echo "    - ${origin}" >> $STARRY_DIR/config.yaml
  done
fi
  ...
  case "$(awk -F: -v u="${SSH_USER}" '$1==u{print $2}' /etc/shadow 2>/dev/null)" in
    ...
    *)
      ...
      if [ -n "$_pw_hash" ] && usermod -p "$_pw_hash" "${SSH_USER}" 2>/dev/null; then
        echo "[ssh] chpasswd 未生效，已改用 usermod -p 写入密码" >> $STARRY_DIR/service.log
      else
        echo "[ssh] 警告: ${SSH_USER} 的密码哈希仍是锁定态，密码登录必然失败" >> $STARRY_DIR/service.log
      fi
      ;;
  esac
fi
...
if [ ! -f /etc/ssh/sshd_config ]; then
  ...
  echo "[ssh] /etc/ssh/sshd_config 不存在，openssh-server 多半没装成，本次不启动 sshd" >> $STARRY_DIR/service.log
else
  ...
  ' /etc/ssh/sshd_config > /etc/ssh/sshd_config.starry-tmp &&
    mv -f /etc/ssh/sshd_config.starry-tmp /etc/ssh/sshd_config
  ...
  if grep -qE '^[[:space:]]*Include[[:space:]]+/etc/ssh/sshd_config\.d/' /etc/ssh/sshd_config; then
    ...
    } > /etc/ssh/sshd_config.d/00-starry.conf
  fi
  ...
  if [ -f /etc/pam.d/sshd ]; then
    ...
    [ -f /etc/pam.d/sshd.starry-bak ] || cp -f /etc/pam.d/sshd /etc/pam.d/sshd.starry-bak 2>/dev/null || true
    ...
    if ! grep -qE '^session[[:space:]]+required[[:space:]]+pam_permit\.so[[:space:]]*$' /etc/pam.d/sshd 2>/dev/null; then
      ...
      ' /etc/pam.d/sshd > /etc/pam.d/sshd.starry-tmp &&
        mv -f /etc/pam.d/sshd.starry-tmp /etc/pam.d/sshd
      echo "[ssh] 已将 /etc/pam.d/sshd 的 session 栈替换为 pam_permit（原文件备份在 sshd.starry-bak）" >> $STARRY_DIR/service.log
    fi
  fi
  ...
  if [ -n "$_sshd_t" ]; then
    echo "[ssh] sshd -t: $_sshd_t" >> $STARRY_DIR/service.log
  fi
  ...
  if ssh_port_listening; then
    echo "[ssh] 端口 ${SSH_PORT} 已在监听，跳过启动 sshd" >> $STARRY_DIR/service.log
  else
    # sshd.log 滚动只在「确实要重新拉起 sshd」时做。
    # 放在外面的话会踩到这个坑：老 sshd 还开着这个 fd，mv 之后它继续往 .old 写，
    # 新的 sshd.log 永远长不到阈值 —— 滚动再也不会触发，.old 无上限增长。
    if [ -f $STARRY_DIR/sshd.log ]; then
      _ssh_log_size=$(stat -c%s $STARRY_DIR/sshd.log 2>/dev/null || echo 0)
      if [ "${_ssh_log_size:-0}" -gt 1048576 ]; then
        mv -f $STARRY_DIR/sshd.log $STARRY_DIR/sshd.log.old 2>/dev/null
      fi
    fi
    ...
    # 用 nohup + & 交给 init 收养，与 starry-server 的拉起方式一致（chroot 无 PID namespace，
    # 父进程退出后子进程照活）。
    nohup /usr/sbin/sshd -D -e >> $STARRY_DIR/sshd.log 2>&1 &
    echo "[ssh] 已拉起 sshd PID=$! port=${SSH_PORT}" >> $STARRY_DIR/service.log
    ...
    if [ "$_ssh_ok" = "1" ]; then
      echo "[ssh] 端口 ${SSH_PORT} 监听正常" >> $STARRY_DIR/service.log
    else
      echo "[ssh] 警告: 等待 5 秒后端口 ${SSH_PORT} 仍未监听，详见 $STARRY_DIR/sshd.log" >> $STARRY_DIR/service.log
    fi
  fi
  ...
  } >> $STARRY_DIR/service.log
fi

# 避免重复拉起 starry-server
if pgrep -f /usr/local/bin/starry-server >/dev/null 2>&1; then
  echo "starry-server 已在运行" >> $STARRY_DIR/service.log
  exit 0
fi

cd $STARRY_DIR
nohup /usr/local/bin/starry-server > $STARRY_DIR/starry.log 2>&1 &
echo "starry-server 已拉起 PID=$! (port=${PANEL_PORT})" >> $STARRY_DIR/service.log
exit 0
CONTAINER_EOF
chmod +x "$STARTUP" 2>/dev/null

log "进入容器启动 starry-server (flavor=$FLAVOR, panel=$PANEL_PORT, ssh=$SSH_PORT)..."

# 输出重定向进宿主 service.log：容器启动脚本里任何没被显式重定向的报错
# （ruri 自身的错误、脚本里漏掉重定向的命令）原来是直接丢掉的。
"$RURIMA" ruri -p -N -S -A $rootfs "$CTR_SHELL" /tmp/starry-startup.sh >> "$LOG_FILE" 2>&1

sleep 2

# 容器内启动后简单验证
if "$RURIMA" ruri -p -N -S -A $rootfs "$CTR_SHELL" -c "pgrep -f /usr/local/bin/starry-server >/dev/null 2>&1"; then
  log "面板启动成功，访问 http://127.0.0.1:${PANEL_PORT}"
else
  log "!! 面板启动失败，查看 $rootfs/app/Starry-Panel/starry.log"
fi

# ---- 守护代次标记 ----------------------------------------------------------
# 必须在 fork 守护【之前】写。
#
# 为什么需要：本脚本对守护子 shell 没有任何去重手段（只对 starry-server 做了 pgrep 去重），
# 而 action.sh / 文档都在教用户重跑 service.sh —— 跑几次就有几个 while 循环，
# 它们彼此看不见对方的 revive_cooldown，会各自进容器重跑 starry-startup.sh
...
#      同步到宿主 /data/adb/starry-panel/deps-snapshot/，下次开机由上面的逻辑回填。
(
  DEPS_PERSIST="$PERSIST_DIR/deps-snapshot"
  DEPS_CONTAINER="$rootfs/app/Starry-Panel/deps"
  # 面板在线升级期间会写这个哨兵，替换窗口内守护不插手，
  # 免得在「旧进程已退出、新进程还没起来」的空档里把旧版本又拉起来。
  UPDATING_FLAG="$rootfs/app/Starry-Panel/.updating"
  ...
  while true; do
    ...
    if [ ! -f "$UPDATING_FLAG" ] && [ "$revive_cooldown" -le 0 ]; then
      if ! panel_is_running; then
        log "!! 面板进程不在，尝试重新拉起"
        "$RURIMA" ruri -p -N -S -A $rootfs "$CTR_SHELL" /tmp/starry-startup.sh
        revive_cooldown=5
      fi
    fi
