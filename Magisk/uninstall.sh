...
#
# 默认会清理：
#   - 运行中的 starry-server 进程
#   - 容器 rootfs (/data/starry 或 /data/local/starry)，Alpine / Debian 路径相同
#   - 持久化目录 /data/adb/starry-panel
#
# 如需保留数据以便重装后继续用，卸载前先：
...
LOG_TAG="starry-panel-uninstall"
...
#    守护的 argv 继承自 service.sh，`pkill -f starry-server` 根本打不到它；
...
# 1. 停止面板进程
pkill -f "starry-server" 2>/dev/null
sleep 1
pkill -9 -f "starry-server" 2>/dev/null

# 2. 清理 rootfs（除非保留）
if [ -f "$KEEP_FLAG" ]; then
  _log "检测到保留标记 $KEEP_FLAG，跳过 rootfs / 持久化目录清理"
  _log "如需彻底删除：su -c \"rm -rf /data/starry /data/local/starry $PERSIST_DIR\""
else
  for rfs in /data/starry /data/local/starry; do
    if [ -d "$rfs" ]; then
      _log "清理 rootfs: $rfs"
      rm -rf "$rfs"
    fi
...
# 4. 清理历史版本可能写入的其它路径
rm -f /system/etc/init.d/99starry 2>/dev/null
rm -f /data/adb/service.d/starry-panel.sh 2>/dev/null
rm -f /data/local/tmp/starry-panel.* 2>/dev/null
