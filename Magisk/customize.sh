#!/system/bin/sh
##########################################################################
# 满天星面板 Magisk / KernelSU / APatch 模块安装脚本
#
# 方案：借鉴 v2.0.5 的容器方案
...
#   4. 面板后端 starry-server (CGO_ENABLED=0 静态 Go 二进制) 放进容器 /usr/local/bin/
#   5. 运行时由 service.sh 通过 rurima ruri 进入容器启动 starry-server，
#      单端口 5700 由 starry-server 直接托管 API + 前端静态文件 (web_dir)
##########################################################################
...
# rootfs 优先使用 /data/starry（若历史已存在），否则 /data/local/starry
export rootfs=/data/local/starry
if [ -d "/data/starry" ]; then
  export rootfs=/data/starry
fi
...
if [ "$FLAVOR" = "debian" ]; then
  ...
  ROOTFS_URL="https://github.com/KKXM112/starry-panel/releases/latest/download/starry-debian-rootfs-arm64.tar.gz"
else
  CTR_SHELL=/bin/ash
...
# ---- 挑选 starry-server 二进制 ------------------------------------------
# 架构检查已经保证只剩 arm64。build.sh 仍保留 amd64 / all 构建能力（属于构建
# 基建，将来真有了 x86_64 的 rurima 只需改回 CI 参数和上面的架构检查），
# 所以下面照旧清理可能存在的 amd64 产物。
BIN_SUFFIX="arm64"

if [ ! -f "$MODPATH/system/bin/starry-server-${BIN_SUFFIX}" ]; then
  abort "! 模块包缺少 system/bin/starry-server-${BIN_SUFFIX}，无法安装"
fi
...
if [ ! -f "$RURIMA" ]; then
  ...
  ui_print "! 请从 GitHub Release 重新完整下载 starry-panel-magisk ZIP 后再装。"
  abort "! 安装已中止（未对设备上的任何数据做改动）"
fi
...
mv -f "$MODPATH/system/bin/starry-server-${BIN_SUFFIX}" "$MODPATH/system/bin/starry-server"
[ -f "$MODPATH/system/bin/starry-server-arm64" ] && rm -f "$MODPATH/system/bin/starry-server-arm64"
[ -f "$MODPATH/system/bin/starry-server-amd64" ] && rm -f "$MODPATH/system/bin/starry-server-amd64"
...
if [ -d "$INSTALL_BACKUP_DIR" ]; then
  ...
  if [ "$backup_count" -gt 0 ]; then
    if [ -f "$INSTALL_IN_PROGRESS_FLAG" ] || [ ! -d "$rootfs/app/Starry-Panel" ]; then
...
if [ "$INSTALL_BACKUP_READY" != "1" ] && [ -d "$rootfs/app/Starry-Panel" ]; then
  ...
  if ! cp -rf "$rootfs/app/Starry-Panel/." "$INSTALL_BACKUP_DIR/" 2>/dev/null; then
  ...
  snapshot_items=0
  for item in starry.db starry.db-shm starry.db-wal scripts backups .jwt_secret config.yaml panel.log; do
...
源版本:   $current_ver
目标版本: $new_ver
源路径:   $rootfs/app/Starry-Panel
项目数:   $snapshot_items
总大小:   ${snapshot_size:-?}

包含: starry.db (+wal/-shm)、scripts/、backups/、.jwt_secret、config.yaml、panel.log
...
恢复方法（任选其一）：
  ...
  方式 B —— 手动：
    ...
    su -c "cp -rf $PERSIST_BACKUP_DIR/. $rootfs/app/Starry-Panel/"
    # 再点一次动作按钮启动，或重启设备：
    su -c "sh /data/adb/modules/$MODID/action.sh"
...
# ---- 先让存活守护自退 ----------------------------------------------------
# 下面这段停容器的老写法只 pkill 了 starry-server 和 ruri，**打不到守护子 shell**
...
if [ -d "$rootfs" ]; then
  ...
  pkill -f starry-server 2>/dev/null || true
...
if [ -d "$rootfs/app/Starry-Panel" ] && [ "$INSTALL_BACKUP_READY" != "1" ]; then
  ...
  abort "! 面板数据存在但未成功备份，已中止安装以保护数据。请重试或手动备份 $rootfs/app/Starry-Panel"
fi
...
CONTAINER_PROBE_TOKEN="STARRY_CONTAINER_PROBE_OK"
...
probe_out=$("$RURIMA" ruri -p -N -S -A "$rootfs" "$CTR_SHELL" -c 'echo "STARRY_CONTAINER""_PROBE_OK"' 2>"$CONTAINER_PROBE_ERR")
...
mkdir -p "$rootfs/tmp"
DEPS_SCRIPT="$rootfs/tmp/starry-install-deps.sh"
...
cat > /etc/apt/apt.conf.d/99-starry-android << 'APTCONF_EOF'
...
for _src in /etc/apt/sources.list /etc/apt/sources.list.d/debian.sources; do
  [ -f "$_src" ] || continue
  [ -f "$_src.starry-orig" ] || cp -f "$_src" "$_src.starry-orig"
done
...
use_mirror() {
  _host="$1"
  for _src in /etc/apt/sources.list /etc/apt/sources.list.d/debian.sources; do
    [ -f "$_src.starry-orig" ] || continue
    cp -f "$_src.starry-orig" "$_src"
...
for _mirror in mirrors.nju.edu.cn mirrors.tuna.tsinghua.edu.cn mirrors.aliyun.com deb.debian.org; do
  echo "[starry] 正在尝试镜像源: $_mirror"
  use_mirror "$_mirror"
  if apt-get update; then
    _mirror_ok=1
    _mirror_used="$_mirror"
    echo "[starry] 镜像源可用: $_mirror"
    break
  fi
  echo "[starry] 镜像源不可用，换下一个: $_mirror"
done
if [ "$_mirror_ok" = "1" ]; then
  echo "MIRROR=$_mirror_used" > /tmp/starry-deps-status
else
  echo "MIRROR=FAIL" > /tmp/starry-deps-status
  echo "[starry] 所有候选镜像源的 apt-get update 都失败了"
fi
...
apt-get install -y --no-install-recommends ca-certificates || \
  echo "[starry] ca-certificates 安装失败，pip / npm / git 的 https 可能不可用"
...
if ! dpkg-query -W -f='${db:Status-Abbrev}' openssh-server 2>/dev/null | grep -q '^ii'; then
  echo "[starry] openssh-server 未安装完整，正在重试"
...
# 注意【不要】顺手删 /etc/apt/apt.conf.d/99-starry-android：
...
for _node_url in \
  ...
  echo "[starry] 正在下载 Node.js v${NODE_VERSION}: $_node_url"
  ...
  if ! curl -fL --retry 3 --connect-timeout 20 --speed-limit 1024 --speed-time 60 -o "/tmp/${NODE_TARBALL}" "$_node_url"; then
    echo "[starry] 下载失败，换下一个源: $_node_url"
    continue
  fi
  if ! echo "${NODE_SHA256}  /tmp/${NODE_TARBALL}" | sha256sum -c - >/dev/null 2>&1; then
    echo "[starry] SHA256 校验不通过（下载被截断或镜像内容不一致），换下一个源: $_node_url"
    continue
  fi
  if tar -xzf "/tmp/${NODE_TARBALL}" -C /usr/local --strip-components=1 --no-same-owner; then
    _node_ok=1
    echo "[starry] Node.js v${NODE_VERSION} 已解压到 /usr/local"
    break
  fi
  echo "[starry] 解压失败（多半是存储空间不足），换下一个源: $_node_url"
done
rm -f "/tmp/${NODE_TARBALL}"
...
if [ "$_node_ok" = "1" ]; then
  echo "NODE=OK" >> /tmp/starry-deps-status
else
  echo "NODE=FAIL" >> /tmp/starry-deps-status
  echo "[starry] Node.js 官方二进制在 npmmirror 与 nodejs.org 上都没装成"
fi
DEPS_PKG_DEBIAN_EOF
else
...
case "$(awk -F: -v u="${SSH_USER}" '$1==u{print $2}' /etc/shadow 2>/dev/null)" in
  ...
  *)
    ...
    if [ -n "$_pw_hash" ]; then
      echo "[starry] chpasswd 未写入密码哈希，改用 usermod -p 兜底"
...
mkdir -p /app /app/web /app/Starry-Panel
DEPS_COMMON_EOF
...
if [ "$FLAVOR" = "debian" ]; then
  ...
  cat > "$rootfs/tmp/starry-dns-probe.sh" << 'DNS_PROBE_EOF'
#!/bin/bash
...
DNS_PROBE_EOF
  chmod +x "$rootfs/tmp/starry-dns-probe.sh" 2>/dev/null
  DNS_PROBE_OUT="$TMPDIR/dns-probe.txt"
  : > "$DNS_PROBE_OUT"
  "$RURIMA" ruri -p -N -S -A "$rootfs" "$CTR_SHELL" /tmp/starry-dns-probe.sh \
...
"$RURIMA" ruri -p -N -S -A "$rootfs" "$CTR_SHELL" /tmp/starry-install-deps.sh
...
if [ -n "$missing_runtimes" ]; then
  ...
  if [ "$FLAVOR" = "debian" ]; then
    deps_status=""
    node_status=""
    if [ -f "$rootfs/tmp/starry-deps-status" ]; then
      deps_status=$(grep '^MIRROR=' "$rootfs/tmp/starry-deps-status" 2>/dev/null | cut -d= -f2-)
      node_status=$(grep '^NODE=' "$rootfs/tmp/starry-deps-status" 2>/dev/null | cut -d= -f2-)
    fi
    case "$DNS_PROBE_VERDICT" in
      ...
      apt_uid_blocked)
        ...
        ui_print "! /etc/apt/apt.conf.d/99-starry-android 让 apt 不再降权，若仍失败，"
...
export PATH=/data/adb/starry-panel/bin/python/bin:/data/adb/starry-panel/bin/node/bin:/data/adb/starry-panel/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
...
if [ "$INSTALL_BACKUP_READY" = "1" ] && [ -d "$INSTALL_BACKUP_DIR" ]; then
  ui_print "- 正在恢复用户数据..."
  mkdir -p "$rootfs/app/Starry-Panel"
  for item in "$INSTALL_BACKUP_DIR"/* "$INSTALL_BACKUP_DIR"/.[!.]* "$INSTALL_BACKUP_DIR"/..?*; do
    [ -e "$item" ] || continue
    cp -rf "$item" "$rootfs/app/Starry-Panel/" 2>/dev/null || \
...
#!/system/bin/sh
# 满天星面板 - 一键恢复"上次更新前"的数据快照。
# 使用：su -c "sh /data/adb/starry-panel/restore-last-update.sh"
set -e

MODID=$MODID
PERSIST_DIR=$PERSIST_DIR
BACKUP_DIR="\$PERSIST_DIR/last-update-backup"
ROOTFS_CANDIDATES="/data/starry /data/local/starry"
...
for candidate in \$ROOTFS_CANDIDATES; do
  if [ -d "\$candidate/app/Starry-Panel" ] || [ -d "\$candidate/app" ]; then
    ROOTFS="\$candidate"
    break
  fi
done
...
TARGET="\$ROOTFS/app/Starry-Panel"
log "rootfs: \$ROOTFS"
log "目标: \$TARGET"
...
ui_print "- 数据目录:   $rootfs/app/Starry-Panel"
ui_print ""
