...
**本地缓存**
- **沿用老键和老格式**：`dd:tasks:page_size`（数字字符串）、`starry-env-page-size`（`'20'|'50'|'100'|'all'`）、
...
- 🔴 **所有写入必须经 `setListPreference(s)`**。验收 grep：
  `dd:tasks:page_size|starry-env-page-size|dd:tasks:view_(all|groups)_hidden` 在 `web/src` 里只允许出现在 `listPreferences.ts`（v3.3.1 时零违例）。
