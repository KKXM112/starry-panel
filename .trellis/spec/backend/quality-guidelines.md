...
- 运行时环境：`STARRY_PYTHON_RUNTIME_MODE`、`STARRY_PYTHON_VERSION`
- Compose 环境：`STARRY_PANEL_IMAGE` -> `image` + `IMAGE_NAME`
...
- Debian 所有变体和 Alpine 64 位变体只使用 `/opt/starry-python` 独立运行时；all 镜像只能有三套目标 Python，不能再附带系统 Python。
...
- Compose 的实际 `image` 与容器内 `IMAGE_NAME` 必须共用 `STARRY_PANEL_IMAGE`；Watchtower API 地址使用稳定服务名 `http://watchtower:8080`。
...
- `SupportedPythonVersions()` 在 `STARRY_PYTHON_RUNTIME_MODE=single` 时只返回当前版本。
...
Watchtower -> /v1/update?async=true&container=^starry-panel$
...
- 原因：仓库源码目录如果长期保留手工构建或调试阶段的 `server/starry-panel.exe`，很容易和当前源码脱节，导致“源码已修复，但本地 exe 仍是旧行为”。

### 2. Signatures

- GitHub Release Windows 构建：`.github/workflows/release.yml`
- Windows 正式产物名：`starry-server.exe`
- 仓库开发态忽略：`.gitignore` 中应忽略 `server/starry-panel.exe`、`server/ddp.exe`

### 3. Contracts

- 仓库源码目录中的本地 Windows 可执行文件不作为可信发布产物。
- 正式 Windows 发布包必须以 release workflow 使用 `-ldflags "-X starry-panel/handler.Version=..."` 产出的 zip 为准。
- 本地开发产生的 `server/starry-panel.exe`、`server/ddp.exe` 必须被 `.gitignore` 忽略，避免把旧二进制误提交到仓库。

### 4. Validation & Error Matrix

- 源码 `handler.Version` 已更新，但本地 exe 行为仍是旧接口 / 旧版本 → 优先检查是否误用了仓库里旧 exe，而不是当前源码构建产物。
- 工作树中出现 `server/starry-panel.exe` 脏改动 → 视为发布一致性风险，不要混进功能提交。
...
```text
源码修完后直接使用仓库里已有的 server/starry-panel.exe 做验收或发版
```
...
```sh
python3 -m venv "$STARRY_DIR/deps/python/3.12"
```
...
python3 -m venv "$STARRY_DIR/deps/python/$PY_MINOR"
```
...
- 开关常量：`const hookEnvDumpPathEnvKey = "STARRY_HOOK_ENV_DUMP"`
...
- **保护名单 = `TZ` + 全部 `STARRY_` 前缀 + shell 内部易变量。** 前两类是用户**有意**去改的运行时契约（时区链路、脚本令牌、`STARRY_NOTIFY_CHANNEL_ID` 的渠道绑定），拦下来必须往任务日志写一行「已忽略受保护变量: …」，否则用户会一直以为改生效了；`PWD` / `SHLVL` / `IFS` / `BASH_*` / `COMP_*` 这类静默拦，报出来纯属噪音。
...
- **开关是「`STARRY_HOOK_ENV_DUMP` 非空」**且**「同名 `.ok` 标记文件存在」两个条件同时成立**。只有第二道能挡住这种情况：用户在「环境变量」页手建一条同名变量、值随手填成某个真实路径（比如 `/app/config.yaml`），那样每个 bash 任务都会用 `>` 把那个文件截断。`.ok` 只有面板自己会创建，误设的值就只是个空转。
...
- 前置脚本 `export TZ=UTC` / `export STARRY_TOKEN=x` → 被忽略，任务日志出现「已忽略受保护变量: …」
...
- 订阅钩子 / `task_after.sh` / `extra.sh` → 不注入 `STARRY_HOOK_ENV_DUMP`，采集代码整段不装
...
. "$__dd_script" "$@"
__dd_dump_env "$STARRY_HOOK_ENV_DUMP"
```
...
__dd_dump_env "${STARRY_HOOK_ENV_DUMP}.base"
trap '__dd_dump_env "$STARRY_HOOK_ENV_DUMP"' EXIT
. "$__dd_script" "$@"
```
...
- **托管 helper**：
  ...
  - 有类型或名称选择器、又没显式传 `channel_id(s)` 时不回落 `STARRY_NOTIFY_CHANNEL_ID`；显式传了空的类型 / 名称原样发出去由面板 400，不能按真假判断丢掉、悄悄变成广播。
...
| 位置 | 路径 | 在线升级能不能改到 |
|---|---|---|
| 模块本体（宿主 Android 侧） | `/data/adb/modules/starry-panel/` | 能（best-effort，只写三样） |
| 容器 rootfs | `/data/starry` 或 `/data/local/starry` | 不动 |
| 面板实际运行的文件（容器内） | `/usr/local/bin/starry-server`、`/app/web`、`/app/Starry-Panel` | 能 |
| 宿主持久目录 | `/data/adb/starry-panel/`（`ports.conf`、`service.log`、`deps-snapshot/`、`stopped`、`watchdog.gen`） | 不动 |
...
- 外壳版本：Go `const currentMagiskShellVersion`（= service.sh 当前 export 的值）与 `const requiredMagiskShellVersion`（在线升级放行的最低值）↔ shell `export STARRY_MAGISK_SHELL_VERSION`
- plan 新增字段：`DataDir` / `WebDir` / `ModuleDir`
- 升级窗口哨兵：`<DataDir>/.updating`（Go 常量 `magiskUpdatingSentinelName` ↔ service.sh 的 `UPDATING_FLAG`）
- 手动停止开关：`/data/adb/starry-panel/stopped`（Go 常量 `magiskStopFlagPath` ↔ 四个 shell 的 `STOP_FLAG` / `$PERSIST_DIR/stopped`）
- 守护代次标记：`/data/adb/starry-panel/watchdog.gen`（Go 常量 `magiskWatchdogGenName` ↔ service.sh 的 `WATCHDOG_GEN_FILE`）
...
- **升级范围严格限定三样**：`starry-server`、`ddp`、前端目录。容器 rootfs、apt/apk 系统包、Python venv、`config.yaml`、`ports.conf` 一概不动。更新包里自带的 `config.yaml` 必须跳过——它会覆盖模块生成的端口配置。
- **进程路径与名字必须是 `/usr/local/bin/starry-server`**（这条 argv0 是三方共同依赖的契约）：容器启动脚本用 `pgrep -f /usr/local/bin/starry-server` 去重；`service.sh` 的守护（`panel_is_running`）与 `action.sh` 的 `panel_pids` 逐个读 `/proc/<pid>/cmdline` 按这条前缀匹配；Go 侧 `findMagiskPanelServerPID` 拿 argv0 与 `magiskPanelBinaryPath` 做全等比较。改名会让下次开机再拉起一个实例抢同一个端口，也会让动作按钮永远探不到面板、停止功能直接失效。
  - 注意 `action.sh` / `service.sh` **不用 `pkill -f starry-server` 停面板**：执行它的 `sh -c` 自身 cmdline 就含这串字符、会被自己命中。停止路径必须先探到 PID 再 `kill -TERM` / `kill -KILL`。
...
- **运行态判定只校验目录、不校验文件名**：`ddp` 装在 `/usr/local/bin/ddp`，写死 `starry-server` 会让 CLI 分支全成死代码。相应地 plan 必须记录真正的面板 PID，CLI 发起时由 helper 显式 `kill -TERM`，且此时**不得**自杀（会截断 CLI 输出）。
- **`os.Executable()` 要剥掉 `" (deleted)"` 后缀**：二进制被替换后 `/proc/self/exe` 会带这个后缀。
- **外壳版本是两个常量，别当成一个**：
  - `STARRY_MAGISK_SHELL_VERSION`（service.sh）↔ `currentMagiskShellVersion`（Go）：**每改一次 `Magisk/*.sh` 或 rootfs 结构就一起加一**，`magisk_assets_test.go` 静态断言两者逐字相等。它只描述「仓库里的外壳长什么样」。
...
- **手动停止开关（v2 外壳）的四条契约**：
  - 路径固定 `/data/adb/starry-panel/stopped`。**绝不能**放进 `$rootfs/app/Starry-Panel/`：那里的 `.updating` 每次开机被无条件删除，同目录的跨重启标记迟早被同类清理误伤；rootfs 重装还会整体删除它。
...
- **守护子 shell 必须有代次去重**。`service.sh` 只对 `starry-server` 做了 pgrep 去重，对自己 fork 的守护没有任何去重手段；文档与 `action.sh` 又在教用户重跑 `service.sh`。做法是 fork 前写 `watchdog.gen`，守护每轮比对、值变即自退。结束守护**不能**用 `pkill -f service.sh`（会误杀正在执行的 service.sh 本身）。
...
- `STARRY_MAGISK_SHELL_VERSION` 缺失或小于 required -> 不生成 plan，返回「请重新刷入模块 zip」。
- Release 缺少本机架构的 `starry-linux-<arch>.tar.gz` -> 明确报缺哪个包。
...
- helper 脚本必须包含 `TARGET_BIN='/usr/local/bin/starry-server'`、`mv -f "$TARGET_BIN.new" "$TARGET_BIN"`，且 `cd "$DATA_DIR"` 出现在 `nohup "$TARGET_BIN"` **之前**。
...
- `service.sh` 必须包含 `file_needs_sync`、`panel_is_running`、`UPDATING_FLAG`，且 `STARRY_MAGISK_SHELL_VERSION` 与 `currentMagiskShellVersion` 逐字一致；同时断言 `requiredMagiskShellVersion <= currentMagiskShellVersion`。
...
case "$proc_cmdline" in
  /usr/local/bin/starry-server*) return 0 ;;
esac
```

```sh
# 错误：无条件覆盖，会把面板内在线升级的结果在下次开机悄悄回滚掉。
cp -f $MODDIR/system/bin/starry-server $rootfs/usr/local/bin/starry-server
```
...
if file_needs_sync "$MODDIR/system/bin/starry-server" "$rootfs/usr/local/bin/starry-server"; then
  cp -f "$MODDIR/system/bin/starry-server" "$rootfs/usr/local/bin/starry-server"
fi
```
...
- **跨层契约**：entrypoint 的 `STARRY_HOME` 必须与 Go 侧 `resolveWritableHome` 的回落目录
...
adduser -D -H -u "${TARGET_UID}" -G starry starry
su-exec "${RUN_AS_USER}" env "HOME=${STARRY_HOME}" /app/starry-server &
```
...
mkdir -p "${STARRY_HOME}" 2>/dev/null || true
chown -R "${TARGET_UID}:${TARGET_GID}" "${DATA_DIR}" /tmp 2>/dev/null || true
RUN_AS_SPEC="${TARGET_USER}:${TARGET_GID}"
su-exec "${RUN_AS_SPEC}" /usr/bin/env "HOME=${STARRY_HOME}" /app/starry-server &
```
...
- 改 `Magisk/*.sh` 必须同步 `STARRY_MAGISK_SHELL_VERSION` 与
...
- 标记文件：`<Data.Dir>/deps/nodejs/.starry-node-abi`，内容为 `process.versions.modules`（常量 `nodeABIMarkerFileName`）。

### 3. Contracts

- **Python 迁移只收敛「旧且不存在」的版本**：以 `STARRY_PYTHON_VERSION` 为当前版本 C（必须确有解释器），只把**比 C 旧、且解释器确实不存在**的小版本 V 的 `dependencies.python_version` / `tasks.python_version` / `python_default_version` 改成 C。比 C 新的一律不动 —— Debian flavor 的系统 python3 是 3.11，用户显式选的 3.12 要靠一键安装补回，不能被降级。V 仍可用时也不动。
...
- 迁移侧：非模块运行态、`STARRY_PYTHON_VERSION` 为空或不在 3.10–3.12、当前版本探测不到解释器 → 一律不动任何记录。
...
| 2 | Alpine / apk | Alpine 镜像（musl）跑不了 Playwright 官方的 Chromium（glibc 构建），请换 Debian 版镜像 KKXM112/starry-panel:debian |
