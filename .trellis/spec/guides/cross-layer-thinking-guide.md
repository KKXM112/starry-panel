...
- `Magisk/service.sh` 导出的环境变量 ↔ Go 里读它的地方（`STARRY_MAGISK_MODULE`、`STARRY_MAGISK_SHELL_VERSION`、`STARRY_ANDROID_RUNTIME_BIN_DIR`）
- Go 写出的文件名 / 哨兵 ↔ shell 里判断它的地方（例如升级窗口哨兵 `.updating`）
- 外壳版本号：改任何 `Magisk/*.sh` 或 rootfs 结构，`STARRY_MAGISK_SHELL_VERSION` 与 Go 里的 **`currentMagiskShellVersion`** 必须**一起加一**（后者的定义就是「本仓库 service.sh 当前 export 的值」，`magisk_assets_test.go` 静态断言两者相等）。
...
APP 在**另一个仓库**（`D:\GitHub\Starry Panel\android-app`，Flutter，iOS/Android 共用一套代码），
...
本项目最终用的是 `web/src/demo/index.ts` 里一个纯 ASCII 字符串字面量哨兵 `__STARRY_DEMO_MOCK__`。
...
| `npm run build:demo` | 浏览器内 mock 顶替层、`robots=index, follow`、资源带 `/starry-panel/` 前缀 |
