...
面板在执行定时任务时，会往脚本进程里注入一批 `STARRY_*` 环境变量，其中包含**面板自己的接口地址和一枚临时凭据**。
脚本因此可以在运行过程中回头调用面板：发通知、读写环境变量、查任务和日志、触发订阅拉取……全都不需要你去后台申请 Open API 应用，也不用把账号密码写进脚本。

这篇文档写给**写定时任务脚本的人**，不需要懂面板内部实现。

> 一句话版本：`$STARRY_API_BASE` 是接口根地址，`$STARRY_TOKEN` 是配套的令牌，两个拼起来直接发 HTTP 请求就行。
...
| 变量 | 含义 |
|------|------|
| `STARRY_API_BASE` | 面板 API 根地址，形如 `http://127.0.0.1:5701/api/v1`。端口跟着 `config.yaml` 的 `server.port` 走，**不要在脚本里写死** |
| `STARRY_TOKEN` | 调用上面这些接口用的 Bearer 令牌。权限与有效期见[第 6 节](#6-安全说明) |
| `STARRY_NOTIFY_URL` | 发通知接口的完整地址，等价于 `$STARRY_API_BASE/notifications/send`。为兼容既有脚本保留 |
| `STARRY_NOTIFY_TOKEN` | 与 `STARRY_TOKEN` **完全同值**，同样为兼容既有脚本保留。新脚本用哪个都行 |
| `STARRY_NOTIFY_TIMEOUT` | 内置通知 helper 的请求超时，固定 `15000`（毫秒） |
| `STARRY_NOTIFY_CHANNEL_ID` | 当前任务在「通知渠道」里选定的默认渠道 ID。**任务没有选渠道时这个变量不存在**，内置 helper 会退回广播 —— 广播只发到「默认推送」渠道，设为「绑定推送」的渠道收不到 |
| `STARRY_SCRIPTS_DIR` | 脚本根目录的绝对路径（面板里「脚本管理」看到的那个根） |
| `STARRY_NOTIFY_PY` | 内置 Python 通知 helper `notify.py` 的绝对路径 |
| `STARRY_SEND_NOTIFY_JS` | 内置 Node 通知 helper `sendNotify.js` 的绝对路径 |
| `STARRY_PYTHON_VERSION` | 当前任务实际使用的 Python 小版本，如 `3.12`。任务表单里选的版本若在本机不可用，这里是回退后的**真实**版本 |
| `STARRY_SILENT_EXIT_DETECT` | **由你按需设置**（不是面板注入的）。填 `0` / `off` / `false` 可为该任务单独关掉「半路静默结束」检测，见下方小节 |
...
1. **上表里的 `STARRY_*` 是保留名**（`NO_PROXY` / `no_proxy` 不在此列，它走合并追加，见第 4 点；`STARRY_SILENT_EXIT_DETECT` 本来就是你自己设的）。注入发生在面板环境变量之后，如果你在「环境变量」页面建了一条同名的 `STARRY_NOTIFY_URL`，任务运行时会被上面的值覆盖掉。
2. **脚本调试运行、`ddp python` / `ddp shell` 也有。** 这两条路径同样会注入这批变量（但没有 `STARRY_NOTIFY_CHANNEL_ID`，因为它们不挂在任何任务上），令牌也同样在运行 / 会话结束后立即作废，详见[第 6 节](#6-安全说明)。
...
4. **`NO_PROXY` 的注入策略和上面两类都不一样。** 面板一共有三种注入策略，别搞混：
   ...
   **为什么脚本回调面板自身不会走代理**：面板一旦在「系统设置 → 代理设置」里配了代理地址，`HTTP_PROXY` / `HTTPS_PROXY` / `ALL_PROXY` 及其小写版共 6 个变量会一并注入脚本进程 —— 这是为了让脚本里的 `pip` / `npm` / `git` 能走代理。但 Python 的 `urllib` / `requests` 默认也认这些变量，于是连发往 `$STARRY_API_BASE`（`http://127.0.0.1:<端口>/api/v1`）的通知请求都会被交给代理，代理去连一个它根本到不了的「本机」，回一个 `502`。把回环地址放进 `NO_PROXY` 就是为了堵住这条路，同时不影响 `pip` / `npm` / `git` 继续走代理。
...
| 方式 | 作用范围 | 怎么用 |
|------|----------|--------|
| `globalThis.starryDone?.()` | 单个脚本 | 主流程末尾加一行，声明「我跑完了」，之后一律判 CLEAN |
| `STARRY_SILENT_EXIT_DETECT=0` | 单个任务 | 在该任务的环境变量里加这条，只关它一个 |
...
| 退出前 `await globalThis.starryFlush?.()` | **0 / 40** | — |
...
```js
await globalThis.starryFlush?.();   // 等 stdout 真正写完
process.exit(0);
```

写 `?.()` 而不是 `()`，这样脚本在面板之外（本地、青龙）跑时不会因为没有这个函数而报错 —— 和上面的 `globalThis.starryDone?.()` 是同一个道理。

**最省事的办法其实是：别调 `process.exit()`。** 让主流程自然跑完，Node 会自己等写完再退出（上表 0/40）。只有在「还有定时器 / 长连接吊着事件循环、不主动退就一直挂着」时才需要 `process.exit()`，那时才配上 `starryFlush`。
...
两个 helper 由面板自动放进脚本根目录并加进 `PYTHONPATH` / `NODE_PATH`，`import` / `require` 直接能找到，签名与青龙保持兼容。它们内部读的就是 `STARRY_NOTIFY_URL` 和 `STARRY_NOTIFY_TOKEN`。
...
- 按类型发（分渠道函数、`send_to`、`channel_type`）**只发「默认推送」渠道**；设成「绑定推送」的渠道要用 `channel_name` 或 `channel_id` 点名。带了类型或名称时不再使用任务默认渠道 `STARRY_NOTIFY_CHANNEL_ID`。
...
API_BASE = os.environ["STARRY_API_BASE"]
TOKEN = os.environ["STARRY_TOKEN"]
...
```js
const API_BASE = process.env.STARRY_API_BASE;
const HEADERS = {
  Authorization: `Bearer ${process.env.STARRY_TOKEN}`,
...
AUTH="Authorization: Bearer ${STARRY_TOKEN}"
JSON='Content-Type: application/json'

# 1. 读：查名字里含 DEMO_TOKEN 的环境变量（keyword 是模糊匹配）
echo "--- 当前记录 ---"
curl -sS -H "$AUTH" "${STARRY_API_BASE}/envs?all=1&keyword=DEMO_TOKEN"
echo
...
curl -sS -X POST -H "$AUTH" -H "$JSON" \
  ...
  "${STARRY_API_BASE}/notifications/send"
echo
```
...
- 根地址：`$STARRY_API_BASE`（即 `http://127.0.0.1:<后端端口>/api/v1`）
- 鉴权：请求头 `Authorization: Bearer $STARRY_TOKEN`
...
> 注：面板同时保留了不带版本号的 `/api/...` 路径（等价于 `/api/v1/...`），脚本里用 `$STARRY_API_BASE` 即可，不必关心。
>
> 内置 helper 的 `ignore_default_config=True`（Node 侧是 `{ ignore_default_config: true }`）**不是「发给所有人」**：
> 它跳过的只是 `STARRY_NOTIFY_CHANNEL_ID`，也就是任务绑定的那个渠道，跳过之后退回广播 ——
...
`ddp task run` 保留给**人在终端里手动跑**用（`docker exec -it starry-panel ddp task run 12`），那种场景下你自己知道正在开第二条线。
...
`$STARRY_TOKEN` 是 operator 角色，能读写：
...
> 如果你在别处看到 `STARRY_TOKEN`，那也是从上面某一条继承下来的（比如脚本起的子进程）。

**请遵守**

- 🚫 **不要打印到日志**。`print(os.environ["STARRY_TOKEN"])` 会让令牌进入执行日志，而日志是可以被导出的
...
| Windows 单机版 zip | 与 `starry-server.exe` 同目录的 `ddp.exe` | ✅ 需保证它在 PATH 里，或用绝对路径 |
...
- 在面板的「环境变量」页面加一条 `STARRY_CONFIG`，值填 `config.yaml` 的绝对路径。它会被注入任务环境，`ddp` 子进程就能读到
...
3. **`TZ` 和所有 `STARRY_` 开头的变量改了不生效。** 它们是面板的运行时契约：`TZ` 决定面板时区，
   `STARRY_TOKEN` / `STARRY_API_BASE` 是脚本调面板接口的凭据，`STARRY_NOTIFY_CHANNEL_ID` 决定任务通知发给哪个渠道。
...
> Windows 单机版更没有对位概念。
> 需要在其它形态下强行启用，设环境变量 `STARRY_QINGLONG_COMPAT=1`（设 `0` 则强制关闭）。
...
满天星面板里没有 `/ql`，所以这类脚本以前会在第一行就 `No such file or directory` 退出。
...
> 这批变量与 `STARRY_*` / `TZ` 那些**保留名语义相反**：它们**只在你没有定义同名变量时才注入**。
...
- [README → 端口与反向代理](../README.md#端口与反向代理)：`STARRY_API_BASE` 里那个端口是怎么来的
