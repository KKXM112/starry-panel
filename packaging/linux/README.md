...
1. 下载二进制 `starry-server` 与前端静态资源到 `/opt/starry-panel`。
2. 复制 systemd 单元：
   ```bash
   sudo cp starry-panel.service /etc/systemd/system/starry-panel.service
   ```
3. （推荐）创建专用用户：
   ```bash
   sudo useradd -r -s /sbin/nologin -d /opt/starry-panel starry
   sudo chown -R starry:starry /opt/starry-panel
   ```
   然后取消 `starry-panel.service` 中 `User=starry` 和 `Group=starry` 的注释。
4. 启动：
   ```bash
   sudo systemctl daemon-reload
   sudo systemctl enable --now starry-panel
   sudo systemctl status starry-panel
   ```
...
```
然后 `sudo systemctl restart starry-panel`。

### 3. 反向代理后 IP 显示为本地

如果面板放在 NAS 反代之后，请配置信任代理 CIDR：
```ini
# /etc/systemd/system/starry-panel.service
Environment=STARRY_TRUSTED_PROXY_CIDRS=127.0.0.1/32,192.168.0.0/16
```

### 4. SQLite 写入失败

确保 `WorkingDirectory` 及其 `data/` 子目录对运行用户可写。如果用 `User=starry`：
```bash
sudo chown -R starry:starry /opt/starry-panel
```
