...
func TestLoadResolvesRelativeDatabasePathBasedOnConfigDir(t *testing.T) {
  ...
  mode: test
database:
  path: Starry-Panel/starry.db
jwt:
  secret: unit-test-secret
data:
  dir: ./Starry-Panel
  scripts_dir: ./Starry-Panel/scripts
  log_dir: ./Starry-Panel/logs
cors:
  origins: []
`
	if err := os.WriteFile(configPath, []byte(body), 0o644); err != nil {
	...
	// 反向断言：绝对不能基于 cwd 解析
	wrongDB := filepath.Join(otherCwd, "Starry-Panel", "starry.db")
...
func TestResolveDataPathJoinsRelativeOnBaseDir(t *testing.T) {
	...
	got := resolveDataPath(base, "Starry-Panel/starry.db")
	want := filepath.Join(base, "Starry-Panel", "starry.db")
	if got != want {
