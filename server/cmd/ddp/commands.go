...
import (
	...
	"starry-panel/model"
	"starry-panel/pkg/crypto"
	"starry-panel/pkg/validator"
...
func runStatus(rt *cliRuntime) error {
	...
	if serverRunning {
		...
		fmt.Println("服务状态: 未检测到运行中的 starry-server 进程")
	}
...
func runRestart(rt *cliRuntime) error {
	...
	fmt.Printf("已向 starry-server (PID %d) 发送重启信号\n", pid)
...
func runBackupRestore(rt *cliRuntime, args []string) error {
	...
	if pid, err := readServerPID(rt.serverPIDFile()); err == nil && isProcessRunning(pid) {
		...
		if findErr == nil && process.Signal(syscall.SIGTERM) == nil {
			fmt.Printf("已自动重启 starry-server (PID %d)，依赖恢复会在新进程启动后继续校验\n", pid)
			return nil
		}
	}

	fmt.Println("未检测到可自动重启的 starry-server 进程，请手动重启面板")
	return nil
}
