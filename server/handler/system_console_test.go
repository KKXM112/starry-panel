...
func TestConsoleRunExecutesCommandAndReportsExitCode(t *testing.T) {
	...
	successID := mustStartConsoleCommand(t, engine, token, "echo starry-console-ok")
	...
	if !strings.Contains(consoleLogText(success), "starry-console-ok") {
...
func TestConsoleClearRemovesRunRecord(t *testing.T) {
	...
	runID := mustStartConsoleCommand(t, engine, token, "echo starry-console-clear")
	awaitConsoleRunDone(t, engine, token, runID)
