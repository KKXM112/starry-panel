...
func TestResolveMagiskShellVersion(t *testing.T) {
	...
	for _, tc := range cases {
		...
		if got := resolveMagiskShellVersion(); got != tc.want {
			t.Fatalf("STARRY_MAGISK_SHELL_VERSION=%q 应解析为 %d，实际 %d", tc.raw, tc.want, got)
		}
	}
}
...
func TestRewriteMagiskModulePropOnlyTouchesVersionLines(t *testing.T) {
	...
	original := strings.Join([]string{
		"id=starry-panel",
		"name=满天星面板 (Starry Panel)",
		"version=v3.0.2",
		"versionCode=30002",
		"author=KKXM112",
		"description=在 Android 设备上以 root 权限运行满天星面板",
	...
	if !strings.Contains(text, "id=starry-panel") || !strings.Contains(text, "author=KKXM112") {
		t.Fatalf("其余字段被破坏:\n%s", text)
	}
...
func TestMagiskUpdateHelperScriptKeepsCriticalConstraints(t *testing.T) {
	...
	plan := &panelUpdatePlan{
		...
		BinaryName:     "starry-linux-arm64",
		WebDir:         "/app/web",
		DataDir:        "/app/Starry-Panel",
		CurrentPID:     4321,
	...
	// 1. 启动路径必须仍是 /usr/local/bin/starry-server，
	//    否则 Magisk/service.sh 的 `pgrep -f /usr/local/bin/starry-server` 去重失效，
	//    下次开机会再拉起一个实例抢同一个端口。
	if !strings.Contains(script, "TARGET_BIN='/usr/local/bin/starry-server'") {
		t.Fatalf("helper 脚本没有把目标程序固定在 /usr/local/bin/starry-server:\n%s", script)
	}
	...
	if !strings.Contains(script, "DATA_DIR='/app/Starry-Panel'") {
...
func TestBuildPanelUpdateTargetForMagisk(t *testing.T) {
	target := buildPanelUpdateTarget(&panelUpdatePlan{
		...
		AssetName:      "starry-linux-arm64.tar.gz",
		BinaryName:     "starry-linux-arm64",
		WebDir:         "/app/web",
		ModuleDir:      "/data/adb/modules/starry-panel",
	})

	if target["deployment_type"] != panelUpdateDeploymentMagisk {
		t.Fatalf("deployment_type 应为 magisk，实际 %v", target["deployment_type"])
	}
	if target["asset_name"] != "starry-linux-arm64.tar.gz" {
