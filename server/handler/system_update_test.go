...
import (
	...
	"starry-panel/model"
	"starry-panel/testutil"

	"github.com/gin-gonic/gin"
)

func TestResolveUpdateImageTargetUsesMirrorForDockerHubImage(t *testing.T) {
	pullImage, mirrorHost, registryURL := resolveUpdateImageTarget("KKXM112/starry-panel:latest", "docker.1ms.run")

	if pullImage != "docker.1ms.run/KKXM112/starry-panel:latest" {
...
func TestResolveBinaryUpdateDownloadURLUsesConfiguredProxy(t *testing.T) {
	...
	assetURL := "https://github.com/KKXM112/starry-panel/releases/download/v2.2.17/starry-linux-amd64.tar.gz"
...
func TestResolveUpdateImageTargetStripsExplicitDockerHubHost(t *testing.T) {
	pullImage, mirrorHost, registryURL := resolveUpdateImageTarget("docker.io/KKXM112/starry-panel:latest", "docker.1ms.run")
...
func TestNormalizePanelUpdateImageNameUsesRollingDebianTag(t *testing.T) {
	got := normalizePanelUpdateImageName("KKXM112/starry-panel:1.9.8-debian")
	if got != "KKXM112/starry-panel:debian" {
		t.Fatalf("expected debian rolling tag, got %q", got)
	}
}

func TestNormalizePanelUpdateImageNameUsesRollingLatestTag(t *testing.T) {
	got := normalizePanelUpdateImageName("docker.io/KKXM112/starry-panel:1.9.8")
	if got != "docker.io/KKXM112/starry-panel:latest" {
...
func TestNormalizePanelUpdateImageNamePreservesRuntimeVariants(t *testing.T) {
	const repository = "KKXM112/starry-panel:"
	cases := []struct {
		name    string
		input   string
		want    string
...
func TestNormalizePanelUpdateImageNameKeepsUnknownOfficialTag(t *testing.T) {
	const imageName = "KKXM112/starry-panel:preview-arm64"
...
func TestNormalizePanelUpdateImageNameKeepsDigestPinned(t *testing.T) {
	const imageName = "KKXM112/starry-panel@sha256:0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef"
	if got := normalizePanelUpdateImageName(imageName); got != imageName {
...
func TestSupportsDockerSocketPanelUpdateOnlyAllowsFullRollingTags(t *testing.T) {
	cases := []struct {
		...
		{"official latest full", "KKXM112/starry-panel:latest-full", true},
		{"official debian full", "KKXM112/starry-panel:debian-full", true},
		{"normalized fixed latest full", normalizePanelUpdateImageName("KKXM112/starry-panel:2.4.0-full"), true},
		{"normalized fixed debian full", normalizePanelUpdateImageName("KKXM112/starry-panel:2.4.0-debian-full"), true},
		{"official latest slim", "KKXM112/starry-panel:latest", false},
		{"official debian slim", "KKXM112/starry-panel:debian", false},
		{"official python variant", "KKXM112/starry-panel:latest-3.12", false},
		...
		{"untagged image", "KKXM112/starry-panel", false},
	}
...
func TestFormatPanelUpdatePullErrorAddsNetworkHint(t *testing.T) {
	plan := &panelUpdatePlan{
		ImageName:     "KKXM112/starry-panel:latest",
		PullImageName: "docker.1ms.run/KKXM112/starry-panel:latest",
...
func TestCollectVolumeMappingsKeepsCustomBindPath(t *testing.T) {
	info := &dockerInspectInfo{
		HostConfig: dockerInspectHostConfig{
			Binds: []string{
				"/srv/panel-data:/app/Starry-Panel",
			},
		},
		Mounts: []dockerInspectMount{
			{Type: "bind", Source: "/srv/panel-data", Destination: "/app/Starry-Panel", RW: true},
	...
	if got[0] != "/srv/panel-data:/app/Starry-Panel" {
...
func TestCollectVolumeMappingsPreservesNamedVolumeAlongsideBind(t *testing.T) {
	info := &dockerInspectInfo{
		...
		Mounts: []dockerInspectMount{
			{Type: "volume", Name: "starry_panel_data", Destination: "/app/Starry-Panel", RW: true},
			{Type: "bind", Source: "/var/run/docker.sock", Destination: "/var/run/docker.sock", RW: true},
		},
	}
	...
	if _, exists := gotSet["starry_panel_data:/app/Starry-Panel"]; !exists {
...
func TestCollectVolumeMappingsDeduplicatesEquivalentRWBindings(t *testing.T) {
	info := &dockerInspectInfo{
		HostConfig: dockerInspectHostConfig{
			Binds: []string{
				"/srv/panel-data:/app/Starry-Panel:rw",
	...
	if _, exists := gotSet["/srv/panel-data:/app/Starry-Panel:rw"]; !exists {
...
func TestBuildContainerRunArgsPreservesCustomDataDirEnvAndMount(t *testing.T) {
	info := &dockerInspectInfo{
		HostConfig: dockerInspectHostConfig{
			Binds: []string{
				"/opt/starry-data:/srv/custom-data",
				"/var/run/docker.sock:/var/run/docker.sock",
			},
		},
		Config: dockerInspectConfig{
			Env: []string{
				...
				"IMAGE_NAME=KKXM112/starry-panel:2.3.5-debianall",
				"PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",
			},
		},
		Mounts: []dockerInspectMount{
			{Type: "bind", Source: "/opt/starry-data", Destination: "/srv/custom-data", RW: true},
			{Type: "bind", Source: "/var/run/docker.sock", Destination: "/var/run/docker.sock", RW: true},
		},
	}

	got := buildContainerRunArgs("starry-panel", "KKXM112/starry-panel:latest", info)

	if !slices.Contains(got, "/opt/starry-data:/srv/custom-data") {
	...
	if slices.Contains(got, "IMAGE_NAME=KKXM112/starry-panel:2.3.5-debianall") {
		t.Fatalf("expected stale IMAGE_NAME env to be removed, got %v", got)
	}
	if !slices.Contains(got, "IMAGE_NAME=KKXM112/starry-panel:latest") {
		t.Fatalf("expected IMAGE_NAME env to match the actual target image, got %v", got)
	}
	if got[len(got)-1] != "KKXM112/starry-panel:latest" {
...
func TestResolveBinaryReleaseTargetMatchesReleaseAssets(t *testing.T) {
	cases := []struct {
		...
		{"windows", "amd64", "starry-windows-amd64.zip", "starry-server.exe"},
		{"linux", "amd64", "starry-linux-amd64.tar.gz", "starry-linux-amd64"},
		{"linux", "arm64", "starry-linux-arm64.tar.gz", "starry-linux-arm64"},
		{"linux", "386", "starry-linux-386.tar.gz", "starry-linux-386"},
		{"linux", "arm", "starry-linux-armv7.tar.gz", "starry-linux-armv7"},
	}
...
func TestPanelReleaseFindAssetByName(t *testing.T) {
	release := panelReleaseInfo{
		Assets: []panelReleaseAsset{
			{Name: "starry-linux-amd64.tar.gz", BrowserDownloadURL: "https://example.com/linux"},
		},
	}

	asset, ok := release.findAsset("STARRY-LINUX-AMD64.TAR.GZ")
	if !ok {
...
func TestBuildPanelUpdateHelperScriptSkipsInvalidPreviousImageID(t *testing.T) {
	plan := &panelUpdatePlan{
		ContainerName:   "starry-panel",
		PreviousImageID: "KKXM112/starry-panel:latest",
		RunArgs:         []string{"run", "-d", "--name", "starry-panel", "KKXM112/starry-panel:latest"},
	}
...
func TestTriggerWatchtowerUpdateExplainsUnreachableAPI(t *testing.T) {
	...
	if strings.Contains(message, "starry-watchtower") {
...
func TestBuildPanelUpdatePlanInfoPrefersWatchtowerAndKeepsDebianFamily(t *testing.T) {
	...
	t.Setenv("WATCHTOWER_HTTP_API_TOKEN", "demo-token")
	t.Setenv("CONTAINER_NAME", "starry-panel")
	t.Setenv("IMAGE_NAME", "docker.1ms.run/KKXM112/starry-panel:debian-all")
	...
	if plan.ImageName != "docker.1ms.run/KKXM112/starry-panel:debian-all" {
...
func TestBuildPanelUpdatePlanInfoRejectsPinnedWatchtowerImage(t *testing.T) {
	...
	t.Setenv("WATCHTOWER_HTTP_API_URL", "http://watchtower:8080")
	t.Setenv("WATCHTOWER_HTTP_API_TOKEN", "demo-token")
	t.Setenv("IMAGE_NAME", "KKXM112/starry-panel:2.4.0-debian-full")
...
func TestExecutePanelUpdateForCLIPrefersWatchtower(t *testing.T) {
	...
	t.Setenv("WATCHTOWER_HTTP_API_URL", server.URL)
	t.Setenv("WATCHTOWER_HTTP_API_TOKEN", "demo-token")
	t.Setenv("IMAGE_NAME", "KKXM112/starry-panel:latest-full")
	...
	for attempt := 1; attempt <= 2; attempt++ {
		...
		if status.PullImageName != "KKXM112/starry-panel:latest-full" {
...
func TestUpdatePanelReturnsCompletedWatchtowerStatus(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		requests++
		if r.URL.Query().Get("async") != "true" || r.URL.Query().Get("container") != "^starry-panel$" {
	...
	t.Setenv("WATCHTOWER_HTTP_API_TOKEN", "demo-token")
	t.Setenv("CONTAINER_NAME", "starry-panel")
	t.Setenv("IMAGE_NAME", "KKXM112/starry-panel:debian-full")
	panelUpdater = newPanelUpdateManager()
	t.Cleanup(func() { panelUpdater = newPanelUpdateManager() })
...
func TestShouldRequireDockerPanelUpdateIgnoresDockerEnvVarsOutsideContainer(t *testing.T) {
	t.Setenv("IMAGE_NAME", "KKXM112/starry-panel:latest")
	t.Setenv("CONTAINER_NAME", "starry-panel")
...
func TestBuildPanelUpdatePlanForReleaseFallsBackToBinaryWhenDockerEnvVarsLeak(t *testing.T) {
	...
	for _, name := range []string{
		"starry-windows-amd64.zip",
		"starry-linux-amd64.tar.gz",
		"starry-linux-arm64.tar.gz",
		"starry-linux-386.tar.gz",
		"starry-linux-armv7.tar.gz",
	} {
