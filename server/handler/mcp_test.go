...
func TestMCPBasicCredentialsServeReadOnlyToolsThroughOpenAPI(t *testing.T) {
	...
	if info := session.InitializeResult(); info == nil || info.ServerInfo == nil || info.ServerInfo.Name != "starry-panel" {
...
func TestMCPIssue139ToolsReachRealRoutes(t *testing.T) {
	...
	for i := 0; builder.Len() < 120*1024; i++ {
		builder.WriteString("console.log(\"满天星面板 " + strconv.Itoa(i) + "\");\n")
	}
...
func TestMCPServesInitializeBehindReverseProxyHost(t *testing.T) {
	...
	if payload.Result.ServerInfo.Name != "starry-panel" || payload.Result.ProtocolVersion != "2025-06-18" {
