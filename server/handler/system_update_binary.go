...
func resolveBinaryReleaseTarget(goos, goarch string) (assetName, binaryName string, err error) {
	...
	case "windows":
		if goarch == "amd64" {
			return "starry-windows-amd64.zip", "starry-server.exe", nil
		}
	case "linux":
		switch goarch {
		case "amd64":
			return "starry-linux-amd64.tar.gz", "starry-linux-amd64", nil
		case "arm64":
			return "starry-linux-arm64.tar.gz", "starry-linux-arm64", nil
		case "386":
			return "starry-linux-386.tar.gz", "starry-linux-386", nil
		case "arm":
			return "starry-linux-armv7.tar.gz", "starry-linux-armv7", nil
		}
	}
...
func isMagiskPanelRuntime() bool {
	for _, marker := range []string{
		"/data/adb/starry-panel/ports.conf",
		"/data/adb/modules/starry-panel/module.prop",
		"/data/adb/modules_update/starry-panel/module.prop",
	} {
...
func resolveBinaryUpdateBaseDir() string {
	...
	return filepath.Join(os.TempDir(), "starry-panel")
}
...
func downloadBinaryUpdateAsset(assetURL, archivePath string) error {
	...
	req.Header.Set("User-Agent", "starry-panel-updater")
...
for item in "$SRC"/* "$SRC"/.[!.]* "$SRC"/..?*; do
  ...
  name=$(basename "$item")
  case "$name" in
    %s|Starry-Panel|data|logs|backups|.env)
...
try {
  ...
  $Protected = @('%s', 'Starry-Panel', 'data', 'logs', 'backups', '.env')
...
func resolvePanelServerPIDFile() string {
	...
	return filepath.Join(config.C.Data.Dir, "run", "starry-server.pid")
}
