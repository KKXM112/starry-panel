...
//
//	1. 模块本体（宿主 Android 侧）：/data/adb/modules/starry-panel/
//	   里面有 system/bin/starry-server、system/bin/ddp、web/、module.prop。
//	2. 容器 rootfs：/data/starry 或 /data/local/starry。
//	3. 面板真正运行的文件（容器内）：/usr/local/bin/starry-server、/app/web。
//
...
const (
	...
	magiskShellVersionEnv      = "STARRY_MAGISK_SHELL_VERSION"

	// 容器内的固定运行路径。名字和路径都不能变：
	// Magisk/service.sh 与 action.sh 都靠 `/usr/local/bin/starry-server` 这个 argv0
	// 判断面板是否在跑，改名会让开机时重复拉起第二个实例抢同一个端口。
	magiskPanelBinaryPath = "/usr/local/bin/starry-server"
	...
	magiskPersistDir = "/data/adb/starry-panel"
...
var magiskModuleDirCandidates = []string{
	"/data/adb/modules/starry-panel",
	"/data/adb/magisk/modules/starry-panel",
	"/sbin/.magisk/modules/starry-panel",
}

// isMagiskPanelUpdateRuntime 判断当前进程是否跑在 Magisk 模块的容器里。
//
// 判据取合取而不是单看环境变量：误判成模块版会让普通 Linux 部署往
// /usr/local/bin 写文件。STARRY_MAGISK_MODULE 只由 service.sh 生成的容器
// 启动脚本 export（ddp 从面板终端起时同样继承得到）。
//
// 这里只校验「可执行文件在 /usr/local/bin 下」，不校验具体文件名：
// ddp CLI 装在 /usr/local/bin/ddp，写死 starry-server 会让 `ddp check` / `ddp update`
...
func isMagiskPanelUpdateRuntime() bool {
	...
	// 这个链接会变成 "/usr/local/bin/starry-server (deleted)"。旧进程在退出前若还接到一次
...
func syncMagiskModuleDir(plan *panelUpdatePlan, packageRoot string) string {
	...
	if err := copyFilePreservingMode(filepath.Join(packageRoot, plan.BinaryName), filepath.Join(binDir, "starry-server"), 0o755); err != nil {
...
//  2. 新进程必须仍是 /usr/local/bin/starry-server，否则 service.sh 的 pgrep 去重失效。
...
nohup "$TARGET_BIN" >> "$DATA_DIR/starry.log" 2>&1 &
log "新面板进程已拉起 PID=$!"
`,
		plan.CurrentPID,
		plan.ServerPID,
