...
func TestMagiskServiceScriptGuardsOnlineUpgrade(t *testing.T) {
	...
	for _, snippet := range []string{
		`file_needs_sync "$MODDIR/system/bin/starry-server" "$rootfs/usr/local/bin/starry-server"`,
...
func TestMagiskScriptsShareStopFlagPath(t *testing.T) {
	...
	if magiskStopFlagPath != "/data/adb/starry-panel/stopped" {
		t.Fatalf("停止开关路径与 Magisk 脚本约定的 /data/adb/starry-panel/stopped 不一致：%q", magiskStopFlagPath)
	}
	// 停止开关绝不能落在容器数据目录里：service.sh 每次开机都会无条件删掉那里的
	// .updating，同目录的跨重启标记迟早被同类清理误伤；rootfs 重装还会整体删除它。
	if strings.Contains(magiskStopFlagPath, "Starry-Panel") {
	...
	syncIdx := strings.Index(serviceSh, `file_needs_sync "$MODDIR/system/bin/starry-server"`)
	stopIdx := strings.Index(serviceSh, `if [ -f "$STOP_FLAG" ]; then`)
	startIdx := strings.Index(serviceSh, `"$RURIMA" ruri -p -N -S -A $rootfs "$CTR_SHELL" /tmp/starry-startup.sh`)
...
func TestMagiskServiceScriptExportsAndroidRuntimeEnv(t *testing.T) {
	...
	requiredSnippets := []string{
		...
		"/data/adb/starry-panel/bin/python/bin",
		"/data/adb/starry-panel/bin/node/bin",
	}
	...
	requiredSnippets = append(requiredSnippets,
		"export STARRY_MAGISK_SHELL_VERSION="+strconv.Itoa(currentMagiskShellVersion),
	)
	...
	for _, snippet := range []string{
		...
		`"$STARRY_DIR/deps/python/$PY_MINOR"`,
	} {
		if !strings.Contains(text, snippet) {
...
func TestMagiskBuildScriptWritesFlavorFile(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		`OUTZIP="$DIST/starry-panel-magisk${FLAVOR_SUFFIX}-v${VERSION}.zip"`,
...
func TestMagiskCustomizeScriptUsesCapabilityProbeInsteadOfVersionGate(t *testing.T) {
	...
	// 能力探测：哨兵字符串 + 实际进容器执行
	if !strings.Contains(text, "STARRY_CONTAINER_PROBE_OK") {
		t.Fatal("customize.sh 缺少容器能力探测的哨兵字符串")
	}
	// 探测必须排在装依赖之前：装依赖耗时且强依赖网络，
	// 先探测才能把「容器起不来」和「网络不通」两类失败区分开
	probeIdx := strings.Index(text, "STARRY_CONTAINER_PROBE_OK")
	apkIdx := strings.Index(text, "apk add --no-cache")
	if apkIdx < 0 {
...
func TestMagiskCustomizeScriptProbesContainerDNSBeforeInstallingDeps(t *testing.T) {
	...
	// 探测必须排在装依赖之前
	probeIdx := strings.Index(text, "starry-dns-probe.sh")
	installIdx := strings.Index(text, `"$RURIMA" ruri -p -N -S -A "$rootfs" "$CTR_SHELL" /tmp/starry-install-deps.sh`)
...
func TestMagiskCustomizeScriptHardensDebianApt(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		"/etc/apt/apt.conf.d/99-starry-android",
	...
	for i, line := range strings.Split(block, "\n") {
		...
		if strings.Contains(line, "99-starry-android") && strings.Contains(line, "rm ") {
			t.Fatalf("customize.sh Debian 分支第 %d 行不得删除 99-starry-android（运行期 apt 也靠它）: %s",
				i+1, trimmed)
		}
	}
...
func TestMagiskServiceScriptStartsSshdWithPortBasedGuard(t *testing.T) {
	...
	rotateIdx := strings.Index(text, `mv -f $STARRY_DIR/sshd.log $STARRY_DIR/sshd.log.old`)
...
func TestMagiskActionScriptReportsSshStatus(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		`SSHD_LOG="$rootfs/app/Starry-Panel/sshd.log"`,
		`CTR_SERVICE_LOG="$rootfs/app/Starry-Panel/service.log"`,
	} {
		if !strings.Contains(text, snippet) {
...
func TestMagiskAptInstallCommandsParser(t *testing.T) {
	cases := []struct {
		...
		{"|| 之后的 echo 文案不是包名", "apt-get install -y --no-install-recommends ca-certificates || \\\n  echo \"[starry] pip / npm / git 的 https 可能不可用\"", []string{"ca-certificates"}},
		{"echo 里提到 apt-get install 不算", "echo \"[starry] 请手动 apt-get install nodejs npm\"", nil},
...
func TestMagiskCustomizeScriptDebianInstallsOfficialNode24(t *testing.T) {
	...
	appendToStatus := regexp.MustCompile(`>>\s*/tmp/starry-deps-status`)
	var mirrorWrites, nodeWrites []int
	for i, line := range lines {
		if !strings.Contains(line, "/tmp/starry-deps-status") {
			continue
		}
		switch {
		...
		case strings.Contains(line, "NODE="):
			if !appendToStatus.MatchString(line) ||
				strings.Contains(appendToStatus.ReplaceAllString(line, ""), "/tmp/starry-deps-status") {
...
func TestMagiskCustomizeScriptSplitsNodeDownloadFailureHint(t *testing.T) {
	...
	next("NODE= 状态的读取", magiskLineContains(`node_status=$(grep '^NODE=' "$rootfs/tmp/starry-deps-status"`))
...
func TestMagiskServiceScriptDropsNodeABIMarkerMissingFromSnapshot(t *testing.T) {
	...
	cpIdx := magiskIndexOfLine(lines, blockIdx+1, func(line string) bool {
		return strings.HasPrefix(line, `cp -rf "$DEPS_PERSIST/." `) && strings.Contains(line, "/app/Starry-Panel/deps/")
	})
	...
	rmLine := `rm -f "$rootfs/app/Starry-Panel/deps/` + marker + `" 2>/dev/null`
	...
	launchIdx := magiskIndexOfLine(lines, 0, func(line string) bool {
		return strings.HasPrefix(line, `"$RURIMA" ruri -p -N -S -A $rootfs "$CTR_SHELL" /tmp/starry-startup.sh`)
	})
