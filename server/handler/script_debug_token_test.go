...
func TestDebugRunRevokesScriptTokenAfterCompletion(t *testing.T) {
	...
	script := "printf '%s' \"$STARRY_TOKEN\" > " + outputName + "\n"
	...
	if token == "" {
		t.Fatal("expected STARRY_TOKEN to be injected into the debug run environment")
	}
...
func TestBuildScriptExecEnvReturnsRevocableToken(t *testing.T) {
	...
	if envMap["STARRY_TOKEN"] == "" {
		t.Fatalf("expected STARRY_TOKEN in debug env, got %#v", envMap)
	}
