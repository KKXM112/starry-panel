...
func TestRunCodeNormalizesLanguageAndPollsOutput(t *testing.T) {
	...
	result, text := callTool(t, session, "run_code", map[string]any{"code": "ls $STARRY_SCRIPTS_DIR", "language": " Bash "})
	if result.IsError {
		t.Fatalf("run_code 不应失败: %s", text)
	}
	first := fake.recorded()[0]
	if !reflect.DeepEqual(first.body, map[string]any{"code": "ls $STARRY_SCRIPTS_DIR", "language": "bash"}) {
