...
func TestDockerEntrypointHomeMatchesGoFallback(t *testing.T) {
	...
	const shellHome = `STARRY_HOME="${DATA_DIR}/.home"`
	...
	if got := resolveWritableHome(missing, dataDir); got != filepath.Join(dataDir, ".home") {
		t.Fatalf("Go 侧回落目录与 entrypoint 的 STARRY_HOME 不一致，got=%q", got)
	}
}
...
func TestDockerEntrypointPinsHomeWhenDroppingPrivilege(t *testing.T) {
	...
	// starry-server 根本不会被执行，表现成「容器每 2 秒重启、日志只有一个退出码」。
	...
	if !strings.Contains(text, "    /app/starry-server &") {
		t.Fatal("entrypoint.sh 必须保留不降权时的裸启动分支")
	}
}

// 建用户不能再是「声明了家目录却从不创建」：那正是 npm 报
// EACCES: mkdir '/home/starry' 的直接原因。
func TestDockerEntrypointCreatesHomeForRunUser(t *testing.T) {
	...
	if !strings.Contains(text, `mkdir -p "${STARRY_HOME}" 2>/dev/null || true`) {
	...
	for _, forbidden := range []string{
		...
		`useradd -M -u "${TARGET_UID}" -g "${TARGET_GID}" -s /sbin/nologin starry`,
	} {
...
func TestDockerEntrypointSurvivesUidGidCollision(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		`groupmod -g "${TARGET_GID}" starry`,
		`usermod -u "${TARGET_UID}" -g "${TARGET_GID}" -d "${STARRY_HOME}" starry`,
	} {
		if !strings.Contains(text, snippet) {
...
// 配了 PUID 之后 starry-server 是降权跑的，Go 侧 MkdirAll("/ql") 必然 EACCES 静默失败。
...
// 原来会造出一个 uid=0 的假 starry：看起来降了权、实际仍是 root。
...
func TestDockerEntrypointKeepsPrivilegeChangesOptIn(t *testing.T) {
	...
	for _, snippet := range []string{`RUN_AS_USER=""`, `STARRY_HOME=""`} {
