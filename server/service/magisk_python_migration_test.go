...
// STARRY_PYTHON_RUNTIME_MODE 显式清空：宿主机上若设了 single，会让用例混进 Docker 单版本语义。
func setMagiskPythonRuntimeEnv(t *testing.T, current string) {
	t.Helper()
	t.Setenv("STARRY_MAGISK_MODULE", "1")
	t.Setenv("STARRY_PYTHON_VERSION", current)
	t.Setenv("STARRY_PYTHON_RUNTIME_MODE", "")
}
...
// STARRY_PYTHON_VERSION 为空 / 不在支持范围 / 当前版本自己都探测不到解释器时，容器状态不可信，一律不搬。
