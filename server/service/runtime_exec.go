...
// 且**只有 STARRY_HOOK_ENV_DUMP 非空时才装**，其余 bash 任务、订阅钩子、后置钩子
...
__dd_script=$2
shift 2
export STARRY_RUNTIME_SHELL_ENV_FILE="$__dd_env_file"
if [ -f "$__dd_env_file" ]; then
  . "$__dd_env_file"
fi
if [ -n "$STARRY_HOOK_ENV_DUMP" ] && [ -f "${STARRY_HOOK_ENV_DUMP}.ok" ]; then
  __dd_dump_env() {
...
func init() {
	envFile := os.Getenv("STARRY_RUNTIME_ENV_FILE")
	if envFile == "" {
		return
	}
...
func BuildManagedRuntimeEnvMapWithScriptToken(workDir, scriptsDir string, defaultChannelID *uint, ttl time.Duration, pythonVersion string) (map[string]string, *ScriptTokenInfo, error) {
	...
	//
	// ⚠️ 这与紧接着下面的 TZ、以及 STARRY_* 那批「保留名」语义**完全相反**，不要顺手统一掉：
	...
	envMap["STARRY_PYTHON_VERSION"] = pythonVersion
...
func CreateManagedCommand(interpreter, scriptPath string, scriptArgs []string, workDir string, envVars map[string]string) (*exec.Cmd, func(), error) {
	...
	if versionFromInterpreter := ResolvePythonVersionFromInterpreter(interpreter); versionFromInterpreter != "" {
		...
		if envVars != nil {
			envVars["STARRY_PYTHON_VERSION"] = pythonVersion
		}
	}
	runtimePaths := currentManagedRuntimePathsForPythonVersion(pythonVersion)
...
func createManagedGoCommand(scriptPath string, scriptArgs []string, workDir string, envVars map[string]string, runtimePaths managedRuntimePaths) (*exec.Cmd, func(), error) {
	...
	wrapperPath := filepath.Join(filepath.Dir(scriptPath), fmt.Sprintf("000000_starry_env_bootstrap_%d.go", time.Now().UnixNano()))
	...
	cmd := exec.Command(binary, args...)
	cmd.Dir = workDir
	cmd.Env = append(buildBootstrapProcessEnv(envVars), "STARRY_RUNTIME_ENV_FILE="+envFile)
	setPgid(cmd)
	return cmd, cleanup, nil
}
...
func writeManagedRuntimeEnvFile(envVars map[string]string) (string, string, func(), error) {
	tempDir, err := os.MkdirTemp("", "starry-runtime-*")
	if err != nil {
		return "", "", nil, err
	}
...
func writeManagedRuntimeShellEnvFile(envVars map[string]string) (string, string, func(), error) {
	...
	b.WriteString("# starry runtime environment\n")
...
try {
  ...
  const armedAlready = process.env.STARRY_SILENT_EXIT_ARMED === '1';
  process.env.STARRY_SILENT_EXIT_ARMED = '1';
  // 2) 任务级开关：给某个噪音脚本单独配一条同名环境变量填 0 / off / false 即可关掉，
  //    不必为了它把全局配置也关掉、连累其余任务。
  const perTaskSwitch = String(process.env.STARRY_SILENT_EXIT_DETECT || '').toLowerCase();
  ...
  if (!armedAlready && !disabledPerTask && isMainThread) {
    ...
    // 脚本主流程末尾调一次 globalThis.starryDone() 即永久判定为「跑完了」。
    ...
    try {
      Object.defineProperty(globalThis, 'starryDone', {
        value: function starryDone() { declaredDone = true; },
    ...
    process.on('beforeExit', () => {
      ...
      process.stdout.write(
        ...
        '有三种关法：脚本末尾加一行 globalThis.starryDone?.()；' +
        '给该任务加环境变量 STARRY_SILENT_EXIT_DETECT=0；' +
        '或在「系统设置 - 任务运行」里关闭「检测脚本半路静默结束」。\n'
      );
...
func writeNodePreloadScript(tempDir, envFile string, envVars map[string]string, detectSilentExit bool) (string, error) {
	helperPath := filepath.ToSlash(strings.TrimSpace(envVars["STARRY_SEND_NOTIFY_JS"]))
...
// starryFlush：给脚本一个「等 stdout 真正写出去」的手段（#113-3）。
// 用户在汇总输出之后写一行 await globalThis.starryFlush?.() 即可，
// 老面板上没有这个函数时可选链会安静跳过，脚本不用改回去。
// writable 必须是 true：脚本里出现同名变量赋值时，严格模式下给只读属性赋值会抛
// TypeError 打断用户脚本（starryDone 那条是历史写法，不要照抄）。
try {
  Object.defineProperty(globalThis, 'starryFlush', {
    value: function starryFlush() {
