...
func TestNodePreloadDefinesStarryFlush(t *testing.T) {
	tempDir := t.TempDir()
	...
	script := string(raw)
	if !strings.Contains(script, "starryFlush") {
		t.Fatalf("expected starryFlush in preload, got %q", script)
	}
...
// 真跑一遍 node：既验证 preload 能被解析执行，也验证 starryFlush 真的会 resolve，
// 且 writable:true 让脚本里的同名赋值在严格模式下不抛错。
func TestNodePreloadStarryFlushRunsInNode(t *testing.T) {
...
(async () => {
  if (typeof globalThis.starryFlush !== 'function') {
    process.stdout.write('missing');
    return;
  }
  await globalThis.starryFlush();
  // writable:true 才允许脚本里出现同名赋值；写成 false 时这一行在严格模式下会抛 TypeError。
  globalThis.starryFlush = function () {};
...
`
	...
	if got := string(out); got != "flush-ok:1" {
		t.Fatalf("expected starryFlush to resolve and env injection to survive, got %q", got)
	}
}
