...
func qingLongCompatApplicable() bool {
	...
	switch strings.ToLower(strings.TrimSpace(os.Getenv("STARRY_QINGLONG_COMPAT"))) {
	case "1", "true", "on", "yes":
		return true
	case "0", "false", "off", "no":
		return false
	...
	// 那份脚本会 export STARRY_MAGISK_SHELL_VERSION。容器内的 / 是 /data/local/starry
	// 这类真实可写目录，建 /ql 不会碰到 Android 系统分区。
	// 这里刻意用字面量而不是 import handler 的常量：service 不能反向依赖 handler。
	if strings.TrimSpace(os.Getenv("STARRY_MAGISK_SHELL_VERSION")) != "" {
		return true
	}
...
// 在 Docker 下（dataDir=/app/Starry-Panel）算出 /app，于是 $QL_DIR/data/repo 永远指向
