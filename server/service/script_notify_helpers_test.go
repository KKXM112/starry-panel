...
func TestBuildNotifyHelperEnvCreatesManagedHelpers(t *testing.T) {
	...
	if env["STARRY_NOTIFY_URL"] == "" || env["STARRY_NOTIFY_TOKEN"] == "" {
		t.Fatalf("expected notify url/token in env, got %#v", env)
	}
	claims, err := middleware.ParseToken(env["STARRY_NOTIFY_TOKEN"])
	if err != nil {
	...
	for _, path := range paths {
		...
		if !strings.Contains(string(content), "STARRY_PANEL_MANAGED_NOTIFY_HELPER") {
	...
	if got := env["STARRY_SCRIPTS_DIR"]; got != scriptsDir {
		t.Fatalf("expected STARRY_SCRIPTS_DIR=%q, got %q", scriptsDir, got)
	}
...
func TestBuildNotifyHelperEnvUsesAbsoluteHelperPaths(t *testing.T) {
	...
	for _, key := range []string{"STARRY_SCRIPTS_DIR", "STARRY_NOTIFY_PY", "STARRY_SEND_NOTIFY_JS"} {
...
func TestBuildNotifyHelperEnvExposesGenericAPIEntrypoint(t *testing.T) {
	...
	if got := env["STARRY_API_BASE"]; got != wantBase {
		t.Fatalf("expected STARRY_API_BASE=%q, got %q", wantBase, got)
	}
	if got := env["STARRY_NOTIFY_URL"]; got != wantBase+"/notifications/send" {
		t.Fatalf("expected STARRY_NOTIFY_URL to stay unchanged, got %q", got)
	}
	if env["STARRY_TOKEN"] == "" {
		t.Fatalf("expected STARRY_TOKEN to be populated")
	}
	if env["STARRY_TOKEN"] != env["STARRY_NOTIFY_TOKEN"] {
		t.Fatalf("expected STARRY_TOKEN and STARRY_NOTIFY_TOKEN to be the same credential, got %q vs %q",
			env["STARRY_TOKEN"], env["STARRY_NOTIFY_TOKEN"])
	}

	claims, err := middleware.ParseToken(env["STARRY_TOKEN"])
	if err != nil {
		t.Fatalf("parse STARRY_TOKEN: %v", err)
	}
...
func TestManagedRuntimeEnvMapCarriesScriptAPIEntrypoint(t *testing.T) {
	...
	for _, key := range []string{"STARRY_API_BASE", "STARRY_TOKEN", "STARRY_NOTIFY_URL", "STARRY_NOTIFY_TOKEN"} {
		if envMap[key] == "" {
			t.Fatalf("expected %s in task runtime env, got %q", key, envMap[key])
		}
	}

	claims, err := middleware.ParseToken(envMap["STARRY_TOKEN"])
	if err != nil {
		t.Fatalf("parse runtime STARRY_TOKEN: %v", err)
	}
	if claims.ID != tokenInfo.JTI {
...
func helperRunEnv(serverURL string) []string {
	return append(os.Environ(),
		"STARRY_NOTIFY_URL="+serverURL,
		"STARRY_NOTIFY_TOKEN=test-token",
		"STARRY_NOTIFY_TIMEOUT=5000",
		"STARRY_NOTIFY_CHANNEL_ID=7",
...
// 带了类型或名称选择器时不再回落任务默认渠道 STARRY_NOTIFY_CHANNEL_ID；青龙同名分渠道函数都在。
...
`
	...
	cmd.Dir = dir
	cmd.Env = append(helperRunEnv(server.URL), "STARRY_SCRIPTS_DIR="+dir)
	if out, err := cmd.CombinedOutput(); err != nil {
