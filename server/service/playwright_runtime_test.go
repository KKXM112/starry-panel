...
func TestPlanPlaywrightRuntime(t *testing.T) {
	...
	supported := playwrightRuntimeFacts{
		...
		OSRelease:           bookworm,
		PackageManager:      "apt",
		ManagedBrowsersPath: "/app/Starry-Panel/deps/ms-playwright",
		BrowsersPath:        "/app/Starry-Panel/deps/ms-playwright",
	}
	...
	cases := []struct {
		...
		{name: "Windows", facts: with(func(f *playwrightRuntimeFacts) { f.GOOS = "windows" }), reasonHave: []string{"windows"}},
		{
			...
			reasonHave: []string{"Alpine", "KKXM112/starry-panel:debian"},
		},
...
func TestNewPlaywrightBrowserInstallCommand(t *testing.T) {
	...
	playwrightManagedPythonFunc = func(version string) string {
		...
		return "/app/Starry-Panel/deps/python/3.12/bin/python"
	}
...
func TestBuildPlaywrightEnvironmentHint(t *testing.T) {
	...
	const missingLib = "BrowserType.launch: Target page, context or browser has been closed\n" +
		"/app/Starry-Panel/deps/ms-playwright/chromium_headless_shell-1181/chrome-linux/headless_shell: " +
	...
	cases := []struct {
	...
	}{
		{
			...
			env:      playwrightHintEnv{BrowsersPath: "/app/Starry-Panel/deps/ms-playwright", OneClick: true, PendingLinux: noPending},
			contains: []string{"PLAYWRIGHT_BROWSERS_PATH=/app/Starry-Panel/deps/ms-playwright", "依赖管理 → Linux", "安装 Playwright 运行环境"},
		},
		{
			name:        "找不到浏览器（非容器）",
			output:      missingBrowser,
...
func TestBuildRuntimeFailureHintCombinesModuleAndPlaywrightHints(t *testing.T) {
	...
	esm := "Error [ERR_REQUIRE_ESM]: require() of ES Module /app/Starry-Panel/deps/nodejs/node_modules/uuid/dist-node/index.js from /app/Starry-Panel/scripts/wc.js not supported."
