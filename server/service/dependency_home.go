...
// —— su-exec 会按 /etc/passwd 把 HOME 覆写成 /home/starry，而那个目录从来没被创建过
...
func resolveWritableHome(home, dataDir string) string {
	...
	// su-exec 会按 passwd 覆写成 /home/starry），所以收窄到「有值但不可写」
	// 既能覆盖问题，又不会波及上面那类部署。
	if home == "" {
		return home
	}
	...
	// 与 entrypoint.sh 里的 STARRY_HOME 指向同一个位置，两层修复落在同一处，
...
func isWritableDir(dir string) bool {
	...
	probe, err := os.CreateTemp(dir, ".starry-write-probe-")
	if err != nil {
		return false
	}
	name := probe.Name()
