...
func mustParseScriptToken(t *testing.T, envVars map[string]string) *middleware.Claims {
	...
	token := envVars["STARRY_TOKEN"]
	if token == "" {
		t.Fatalf("expected STARRY_TOKEN in task env, got %#v", envVars)
	}
...
func TestRunTaskRevokesScriptTokenAfterCompletion(t *testing.T) {
	...
	if envVars["STARRY_NOTIFY_TOKEN"] != envVars["STARRY_TOKEN"] {
		t.Fatalf("expected STARRY_NOTIFY_TOKEN and STARRY_TOKEN to share one credential")
	}
...
func TestRunTaskKeepsScriptTokenValidWhileRunning(t *testing.T) {
	...
	stubRunCommandWithPlan(t, func(taskEnv map[string]string) (*ScriptResult, error) {
		...
		claims, err := middleware.ParseToken(envVars["STARRY_TOKEN"])
		if err != nil {
			return nil, err
		}
