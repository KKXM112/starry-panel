...
func TestMergeHookEnvExportsProtectsRuntimeContractKeys(t *testing.T) {
	...
	wantIgnored := "STARRY_NOTIFY_CHANNEL_ID,STARRY_TOKEN,TZ"
	...
	if envVars["STARRY_NOTIFY_CHANNEL_ID"] != "7" {
		t.Fatalf("expected notify channel binding to survive, got %q", envVars["STARRY_NOTIFY_CHANNEL_ID"])
	}
	if envVars["STARRY_TOKEN"] != "real-token" {
		t.Fatalf("expected script token to survive, got %q", envVars["STARRY_TOKEN"])
	}
}
...
export TZ=UTC
export STARRY_NOTIFY_CHANNEL_ID=
...
`
	...
	if envVars["STARRY_NOTIFY_CHANNEL_ID"] != "7" {
		t.Fatalf("expected STARRY_* to survive, got %q", envVars["STARRY_NOTIFY_CHANNEL_ID"])
	}
	if len(envVars["BIG_ENV"]) != len(bigValue) {
...
func TestSubscriptionHookStaysUnaffectedByHookEnvCapture(t *testing.T) {
	...
	// 靠 stdout 断言会引入偶发的截断。
	script := `printf 'dump=[%s]\n' "$STARRY_HOOK_ENV_DUMP" > sub-hook.out
...
// `.ok` 门禁：STARRY_HOOK_ENV_DUMP 非空【不足以】装上采集逻辑，还必须有面板落下的
// 同名 .ok 标记文件。
//
