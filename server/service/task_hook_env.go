...
//
// 名字带 STARRY_ 前缀还有第二重作用：它天然落在 hookEnvContractPrefixes 里，
// 前置脚本改它也回传不回来，不会自己把自己的采集路径改掉。
const hookEnvDumpPathEnvKey = "STARRY_HOOK_ENV_DUMP"
...
// hookEnvContractPrefixes 目前只有 STARRY_：面板注入的运行时契约变量，全部不接受回传。
//
// 这里最危险的一条是 STARRY_NOTIFY_CHANNEL_ID —— 前置脚本把它改空，任务通知就会从
// 「定向到绑定渠道」静默退化成广播，脚本里、日志里都不会有任何提示。同理还有
// STARRY_TOKEN / STARRY_API_BASE / STARRY_NOTIFY_* / STARRY_SCRIPTS_DIR /
// STARRY_PYTHON_VERSION，以及 STARRY_RUNTIME_SHELL_ENV_FILE 和上面那个 dump 路径本身。
...
var hookEnvContractPrefixes = []string{"STARRY_"}
...
func newHookEnvCapture(envVars map[string]string) (*hookEnvCapture, error) {
	dir, err := os.MkdirTemp("", "starry-hook-env-*")
	if err != nil {
		return nil, err
	}
