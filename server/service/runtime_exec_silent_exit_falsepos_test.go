...
func TestSilentExitProbeExemptionsWorkOnCommonIdioms(t *testing.T) {
	...
	for _, tc := range idioms {
		...
		// 通道一：脚本末尾声明「我跑完了」。
		t.Run(tc.name+"/starryDone 可豁免", func(t *testing.T) {
			out, exitCode := runNodeWithProbe(t, nodeBin, tc.script+"\nglobalThis.starryDone?.();\n", true)
			if strings.Contains(out, "[任务疑似半路结束]") || exitCode != 0 {
				t.Fatalf("starryDone 没能豁免，exit=%d output=%q", exitCode, out)
			}
		})

		// 通道二：给这个任务单独配环境变量关掉。
		t.Run(tc.name+"/任务级环境变量可豁免", func(t *testing.T) {
			out, exitCode := runNodeWithProbeEnv(t, nodeBin, tc.script, true, []string{"STARRY_SILENT_EXIT_DETECT=0"})
			if strings.Contains(out, "[任务疑似半路结束]") || exitCode != 0 {
...
// starryDone 挂在 globalThis 上，脚本里如果出现同名赋值不能把脚本打断。
func TestSilentExitProbeStarryDoneIsNotBrittle(t *testing.T) {
	nodeBin, err := exec.LookPath("node")
	if err != nil {
		t.Skip("node not found")
	}
	...
	script := `
globalThis.starryDone = function () {};
console.log('脚本没被打断');
`
