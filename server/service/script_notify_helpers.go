...
const (
	managedNotifyHelperToken = "STARRY_PANEL_MANAGED_NOTIFY_HELPER v1"
...
var managedNotifyPyContent = strings.Join([]string{
	...
	"#!/usr/bin/env python3",
	"\"\"\"Starry Panel managed notification helper.",
	"",
	"Usage:",
	...
	"- ignore_default_config=True skips STARRY_NOTIFY_CHANNEL_ID fallback.",
	"- channel_type / channel_name selectors also skip STARRY_NOTIFY_CHANNEL_ID.",
	...
	"- STARRY_NOTIFY_URL: panel notify API URL",
	"- STARRY_NOTIFY_TOKEN: temporary bearer token",
	"- STARRY_NOTIFY_TIMEOUT: timeout in ms or seconds, default 15000ms",
	"- STARRY_NOTIFY_CHANNEL_ID: default notification channel ID for current task",
	"\"\"\"",
	"import json",
	"import os",
	...
	"    raw = timeout if timeout is not None else os.getenv(\"STARRY_NOTIFY_TIMEOUT\", \"15000\")",
	...
	"        return None",
	"    raw = os.getenv(\"STARRY_NOTIFY_CHANNEL_ID\", \"\").strip()",
	"    if not raw:",
	"        return None",
	"    try:",
	"        return int(raw)",
	...
	"        use_default_channel: Whether STARRY_NOTIFY_CHANNEL_ID should be used.",
	"        url: Override STARRY_NOTIFY_URL.",
	"        token: Override STARRY_NOTIFY_TOKEN.",
	"        timeout: Override STARRY_NOTIFY_TIMEOUT.",
	...
	"    \"\"\"",
	"    notify_url = (url or os.getenv(\"STARRY_NOTIFY_URL\", \"\")).strip()",
	"    notify_token = (token or os.getenv(\"STARRY_NOTIFY_TOKEN\", \"\")).strip()",
	"    if not notify_url or not notify_token:",
	"        raise RuntimeError(\"STARRY_NOTIFY_URL 或 STARRY_NOTIFY_TOKEN 未配置\")",
	"",
...
var managedSendNotifyJSContent = strings.Join([]string{
	...
	" * - params.ignore_default_config = true skips STARRY_NOTIFY_CHANNEL_ID.",
	" * - channel_type / channel_name selectors also skip STARRY_NOTIFY_CHANNEL_ID.",
	" *",
	" * Per-channel sending:",
	...
	"]);",
	"const SCRIPTS_DIR = String(process.env.STARRY_SCRIPTS_DIR || __dirname).trim() || __dirname;",
	...
	"  if (global.__STARRY_SEND_NOTIFY_ALIAS_PATCHED__) {",
	"    return;",
	"  }",
	...
	"    return originalResolveFilename.call(this, request, parent, isMain, options);",
	"  };",
	"  global.__STARRY_SEND_NOTIFY_ALIAS_PATCHED__ = true;",
	"}",
	"",
	...
	"  const raw = timeout ?? process.env.STARRY_NOTIFY_TIMEOUT ?? DEFAULT_TIMEOUT_MS;",
	...
	"    return null;",
	"  }",
	"  const raw = String(process.env.STARRY_NOTIFY_CHANNEL_ID || '').trim();",
	"  if (!raw) {",
	"    return null;",
	"  }",
	...
	"  const notifyUrl = String(params.url || process.env.STARRY_NOTIFY_URL || '').trim();",
	"  const notifyToken = String(params.token || process.env.STARRY_NOTIFY_TOKEN || '').trim();",
	"  const timeoutMs = resolveTimeoutMs(params.timeout);",
	"  if (!notifyUrl || !notifyToken) {",
	"    return Promise.reject(new Error('STARRY_NOTIFY_URL 或 STARRY_NOTIFY_TOKEN 未配置'));",
	"  }",
	"",
...
func BuildNotifyHelperEnv(scriptsDir string, workDir string, serverPort int, defaultChannelID *uint, ttl time.Duration) (map[string]string, *ScriptTokenInfo, error) {
	...
	// STARRY_NOTIFY_URL / STARRY_NOTIFY_TOKEN 是历史契约，内置 notify.py、sendNotify.js
	// 以及用户既有脚本都在读，只能新增别名、不能改名。
	// STARRY_API_BASE / STARRY_TOKEN 是通用入口：脚本不必再对 STARRY_NOTIFY_URL
	// 做字符串截断去拼别的接口，两枚 token 是同一枚凭据。
	env := map[string]string{
		"STARRY_NOTIFY_URL":     apiBase + "/notifications/send",
		"STARRY_NOTIFY_TOKEN":   tokenInfo.Token,
		"STARRY_NOTIFY_TIMEOUT": "15000",
		"STARRY_SCRIPTS_DIR":    scriptsDir,
		"STARRY_NOTIFY_PY":      filepath.Join(scriptsDir, notifyPyFilename),
		"STARRY_SEND_NOTIFY_JS": filepath.Join(scriptsDir, sendNotifyJSFilename),
		"STARRY_API_BASE":       apiBase,
		"STARRY_TOKEN":          tokenInfo.Token,
	}
	if defaultChannelID != nil && *defaultChannelID > 0 {
		env["STARRY_NOTIFY_CHANNEL_ID"] = fmt.Sprintf("%d", *defaultChannelID)
	}
