...
RUN GOARM=$(case "${TARGETVARIANT}" in v7) echo 7;; v6) echo 6;; v5) echo 5;; *) echo '';; esac) && \
    ...
    go build -ldflags="-s -w -X starry-panel/handler.Version=${VERSION}" -o starry-server . && \
    CGO_ENABLED=0 GOOS=${TARGETOS} GOARCH=${TARGETARCH} GOARM=${GOARM} \
    go build -ldflags="-s -w -X starry-panel/handler.Version=${VERSION}" -o ddp ./cmd/ddp
...
RUN mkdir -p /app/Starry-Panel/scripts /app/Starry-Panel/logs /app/Starry-Panel/backups /run/nginx /tmp && chmod 1777 /tmp

WORKDIR /app

COPY --from=backend-builder /build/starry-server .
...
ENV LC_ALL=C.UTF-8
ENV PANEL_PORT=5700
ENV STARRY_PYTHON_RUNTIME_MODE=${PYTHON_RUNTIME_MODE}
ENV STARRY_PYTHON_VERSION=${PYTHON_RUNTIME_VERSION}
ENV STARRY_PYTHON_RUNTIME_ROOT=/opt/starry-python

EXPOSE ${PANEL_PORT}

VOLUME ["/app/Starry-Panel"]
