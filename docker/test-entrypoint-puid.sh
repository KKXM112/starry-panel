...
#   /app/starry-server -> 打印 uid/gid/HOME 并真的往 $HOME/.npm 里建目录
#                       （这正是用户报障时 npm 失败的那一步）
##############################################################################
...
if [ "${STARRY_PUID_TEST_ISOLATED:-0}" != "1" ]; then
  exec unshare --mount --fork --propagation private \
    env STARRY_PUID_TEST_ISOLATED=1 ENTRYPOINT_SRC="$SRC" bash "$0" "$@"
fi
...
# starry 账号绝不能碰 —— 仓库自己推荐的二进制部署方式
# （packaging/linux/starry-panel.service 的 User=starry）就会建这么一个服务账号，
# 在那种机器上跑本脚本会把它连同 /etc/shadow 条目一起删掉，面板下次直接起不来。
if id -u starry >/dev/null 2>&1 || getent group starry >/dev/null 2>&1; then
  echo "!! 本机已存在 starry 用户或用户组，本脚本会在收尾时删除同名账号，为避免误删直接退出"
  echo "   （如果这台机器上没有在跑面板，可先 userdel starry / groupdel starry 再重跑）"
  exit 2
fi
...
cleanup() {
  ...
  id -u starry >/dev/null 2>&1 && userdel starry 2>/dev/null
  getent group starry >/dev/null 2>&1 && groupdel starry 2>/dev/null
  return 0
}
trap cleanup EXIT
...
exit 0
STUB
chmod +x "$WORK/bin"/* /app/starry-server
...
userdel starry 2>/dev/null; groupdel starry 2>/dev/null
...
groupadd -g 7000 starry-collide-grp 2>/dev/null || true
useradd -M -u 7000 -g 100 -s /usr/sbin/nologin starry-collide 2>/dev/null || true
if [ -z "$(getent passwd 7000)" ] || [ -z "$(getent group 7000)" ]; then
  echo "!! 无法预置 uid/gid 7000 的撞车场景，这条用例会退化成无冲突路径，判为失败"
  FAILED=1
else
  echo "预置完成：uid 7000 = $(getent passwd 7000 | cut -d: -f1)（主组 gid $(id -g starry-collide)）, gid 7000 = $(getent group 7000 | cut -d: -f1)"
  run_case "PUID=7000 PGID=7000（uid/gid 均已被占用，且现成账号主组≠PGID）" PUID=7000 PGID=7000
  expect_rc0
  expect "复用现成账号运行"                 "SERVER uid=7000"
  # 这一条是关键：passwd 里 starry-collide 的主组是 100，
...
userdel starry-collide 2>/dev/null; groupdel starry-collide-grp 2>/dev/null
userdel starry 2>/dev/null; groupdel starry 2>/dev/null
...
# ---- 4. 改了 PUID 之后 docker restart（容器可写层里已有旧 starry）----------
...
# 这是本轮差点引入的回归：新加的 mkdir "$STARRY_HOME" 一旦不带 || true，
