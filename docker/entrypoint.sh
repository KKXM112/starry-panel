...
DATA_DIR=${DATA_DIR:-/app/Starry-Panel}
SERVER_PID_FILE="${DATA_DIR}/run/starry-server.pid"
...
# 配了 PUID 之后 starry-server 是降权跑的，它去 mkdir /ql 必然 EACCES 且只留一行日志。
...
#      两个参数都是「不创建家目录」，但 /etc/passwd 里的家目录字段照写 /home/starry。
#      而 npm 的 cache（$HOME/.npm）、.npmrc，pip 的 pip.conf、pip --user 落点
#      全都只认 HOME —— /home 是镜像层的 root:root 0755，starry 建不出子目录，
#      装依赖必然 EACCES: mkdir '/home/starry'。这就是用户报障的原始现象。
...
#   3. 只设 PGID 不设 PUID 时 TARGET_UID 取到 0，造出一个 uid=0 的假 starry：
...
if [ -n "${PUID}" ] || [ -n "${PGID}" ]; then
  ...
  if [ "${TARGET_UID}" = "0" ]; then
    ...
    if [ -z "${TARGET_GROUP}" ]; then
      if getent group starry >/dev/null 2>&1; then
        # starry 组已存在但 GID 不是这次要的：用户改了 PGID 之后只做了 docker restart，
        # 容器可写层还在，上一次建的组会原样保留。就地改 GID，不要重建。
        groupmod -g "${TARGET_GID}" starry >/dev/null 2>&1 || true
      elif command -v groupadd >/dev/null 2>&1; then
        groupadd -g "${TARGET_GID}" starry >/dev/null 2>&1 || true
      else
        addgroup -g "${TARGET_GID}" starry >/dev/null 2>&1 || true
      fi
      TARGET_GROUP=$(getent group "${TARGET_GID}" 2>/dev/null | cut -d: -f1)
    fi
    ...
    if [ -z "${TARGET_USER}" ]; then
      if id -u starry >/dev/null 2>&1; then
        usermod -u "${TARGET_UID}" -g "${TARGET_GID}" -d "${STARRY_HOME}" starry >/dev/null 2>&1 || true
      elif command -v useradd >/dev/null 2>&1; then
        useradd -M -d "${STARRY_HOME}" -u "${TARGET_UID}" -g "${TARGET_GID}" -s /sbin/nologin starry >/dev/null 2>&1 || true
      else
        adduser -D -H -h "${STARRY_HOME}" -u "${TARGET_UID}" -G "${TARGET_GROUP:-starry}" starry >/dev/null 2>&1 || true
      fi
      TARGET_USER=$(getent passwd "${TARGET_UID}" 2>/dev/null | cut -d: -f1)
    fi
    ...
    if [ -z "${TARGET_USER}" ]; then
      ...
      log "应用 PUID=${TARGET_UID} PGID=${TARGET_GID}（运行用户 ${TARGET_USER}，HOME=${STARRY_HOME}），正在调整数据目录所有权..."
      chown -R "${TARGET_UID}:${TARGET_GID}" "${DATA_DIR}" /tmp 2>/dev/null || true
      # /ql 也要让降权后的 starry 能写：青龙脚本第一步就是 touch "$QL_DIR/shell/env.sh"，
      # Go 侧还要能重建指错的软链。
      #
...
# --- 数据目录可写性预检 -----------------------------------------------------
WRITE_PROBE="${DATA_DIR}/.starry-write-probe-$$"
PROBE_CMD="true"
...
if [ -n "${RUN_AS_USER}" ] && [ -n "${STARRY_HOME}" ]; then
  HOME_PROBE="${STARRY_HOME}/.starry-write-probe-$$"
  ...
  if sh -c "${HOME_PROBE_CMD}" 2>/dev/null; then
    ...
    log "警告：运行用户 ${RUN_AS_USER} 对 HOME（${STARRY_HOME}）没有写权限，面板内安装 Node.js / Python 依赖会失败"
...
export STARRY_PYTHON_RUNTIME_ROOT="${STARRY_PYTHON_RUNTIME_ROOT:-/opt/starry-python}"
STARRY_PYTHON_BIN_PATH=""
STARRY_PYTHON_LIB_PATH=""
for py_ver in 3.12 3.11 3.10; do
  py_root="${STARRY_PYTHON_RUNTIME_ROOT}/${py_ver}"
  if [ -d "${py_root}/bin" ]; then
    STARRY_PYTHON_BIN_PATH="${STARRY_PYTHON_BIN_PATH:+${STARRY_PYTHON_BIN_PATH}:}${py_root}/bin"
  fi
  if [ -d "${py_root}/lib" ]; then
    STARRY_PYTHON_LIB_PATH="${STARRY_PYTHON_LIB_PATH:+${STARRY_PYTHON_LIB_PATH}:}${py_root}/lib"
  fi
done
DEFAULT_PYTHON_VERSION="${STARRY_PYTHON_VERSION:-3.12}"
export PATH="${DATA_DIR}/deps/nodejs/node_modules/.bin:${DATA_DIR}/deps/python/${DEFAULT_PYTHON_VERSION}/bin:${STARRY_PYTHON_BIN_PATH:+${STARRY_PYTHON_BIN_PATH}:}${PATH}"
if [ -n "${STARRY_PYTHON_LIB_PATH}" ]; then
  export LD_LIBRARY_PATH="${STARRY_PYTHON_LIB_PATH}${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}"
fi
...
#     里的 `path: ./data/starry.db` 这种相对路径占位 COPY 到了 /app/config.yaml，
#     幂等逻辑保留了占位，starry-server 按 cwd 解析得到 /app/data/starry.db，
#     新建空库，旧数据（/app/Starry-Panel/starry.db）找不到 → "面板像刚装好"。
#   v2.2.7：用硬编码字符串 `./data/starry.db` / `dir: ./data` 识别"占位"。
...
scan_legacy_db_locations() {
  ...
  #   1) /app/data/starry.db —— v2.2.6 错位生成的空库/半库
  #   2) 任意自定义挂载点下的旧库 —— 用户用 `-v /host/x:/data` + DATA_DIR=/data
  #      或者类似 /config /opt/starry /share/... 的 NAS 习惯挂载点。
  #
  # 第一阶段：扫已知常见挂载点；第二阶段：浅 find 兜底覆盖任意自定义挂载点。
  # 用临时文件汇总而不是 shell 变量——pipe to while 在 POSIX sh 里跑在子 shell，
  # 父 shell 拿不到变量修改。
  current="${DATA_DIR}/starry.db"
  tmp_scanned=$(mktemp 2>/dev/null || echo /tmp/.starry-scan-$$)
  : > "${tmp_scanned}"
  ...
  for candidate in \
      /app/data/starry.db \
      /app/Starry-Panel/starry.db \
      /data/starry.db \
      /config/starry.db \
      /opt/starry/starry.db \
      /app/starry.db; do
  ...
  if command -v find >/dev/null 2>&1; then
    find / -maxdepth 4 -name 'starry.db' -type f \
...
# HOME=/root，所以 gosu 那条路上 HOME 会原样保持 /root，starry 照样写不进去）。
...
# 裸写 env 就会解析到它，starry-server 根本不会被执行，
...
while true; do
  if [ -n "${RUN_AS_USER}" ] && command -v su-exec >/dev/null 2>&1; then
    ...
    gosu "${RUN_AS_SPEC}" /usr/bin/env "HOME=${STARRY_HOME}" /app/starry-server &
  else
    /app/starry-server &
  fi
  SERVER_PID=$!
  echo "${SERVER_PID}" > "${SERVER_PID_FILE}"
  # PID 文件是 root 写的，starry 覆写会 EACCES —— 后端只打一行日志不影响功能，
  ...
  log "starry-server 异常退出 (code=${EXIT_CODE})，2 秒后重启"
  sleep 2
done
