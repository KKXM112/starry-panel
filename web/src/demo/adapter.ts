...
const DEMO_RELEASE_NOTES = [
  ...
  '- **功能修复**：两个来自用户反馈的 issue，实时日志丢最后几秒（[#102](https://github.com/KKXM112/starry-panel/issues/102)）、Magisk Debian 版 SSH 连不上（[#100](https://github.com/KKXM112/starry-panel/issues/100)）。',
  '',
  '> ⚠️ **Magisk 用户注意**：#100 的修复动的是**模块外壳脚本**，面板内的一键升级只替换 `starry-server` / `ddp` / `web`，**覆盖不到**。要拿到这项修复必须**重刷模块 ZIP**。',
  '> 其余改动（含 #102）随正常更新即可。',
  '',
  '---',
  '',
...
route('GET', '/system/console/meta', () => ({
  ...
  shell: 'bash',
  work_dir: '/app/Starry-Panel/scripts',
  is_root: false,
  package_manager: 'apk',
  distribution: 'alpine',
