...
function buildLoginLogs(now: number): DemoLoginLog[] {
  ...
  return seeds.map((seed, index) => ({
    ...
    username: seed[0],
    ip: seed[3],
    client_name: index === 2 ? '满天星面板 APP' : 'Chrome · Windows',
...
function buildSessions(now: number): DemoSession[] {
  return [
    ...
    {
      id: 2, user_id: 2, username: 'ops', client_type: 'app', client_name: '满天星面板 APP',
...
function buildBackups(now: number): DemoBackupFile[] {
  return [
    { name: 'starry-backup-auto-20260819-0300.tgz', size: 8_412_672, created_at: iso(now - 29 * HOUR_MS) },
    { name: 'starry-backup-auto-20260818-0300.tgz', size: 8_355_840, created_at: iso(now - 53 * HOUR_MS) },
    { name: 'starry-backup-升级前手动备份.tgz', size: 7_982_080, created_at: iso(now - 9 * DAY_MS) },
  ]
}
