    ...
    endpoints: [
      {
        ...
        title: '对接概述',
        description: '满天星面板通过 Open API 应用机制对接第三方插件，实现环境变量的自动管理。首先在面板「Open API」页面创建应用，获取 App Key 和 App Secret。配置格式：Host丨AppKey丨AppSecret（使用中文竖线 丨 分隔），示例：http://127.0.0.1:5173丨a1b2c3d4...丨e5f6g7h8...。对接流程：1. 使用 App Key + App Secret 获取 Token → 2. 使用 Token 操作环境变量（增删改查）→ 3. Token 过期（24小时）后重新获取。',
        auth: 'none',
        ...
        responseFields: [
          { name: 'Host', type: 'string', description: '满天星面板地址，如 http://127.0.0.1:5173' },
      ...
      },
      {
        ...
        path: '/api/envs',
        title: '添加环境变量',
        description: '当查找不到已有变量时，调用此接口添加新的环境变量。注意：满天星面板不需要对 value 进行 URL 编码，直接传递原始值即可。',
        auth: 'jwt',
        bodyParams: [
      ...
      },
      {
        ...
        path: '/api/envs/:id',
        title: '删除环境变量',
        description: '删除指定的环境变量。注意：满天星面板通过路径参数传递 ID，不是请求体数组。',
        auth: 'jwt',
        pathParams: [
...
panel.add_or_update_env(
    ...
    keyword="账号1"
      ...
      },
      {
        ...
        responseExample: JSON.stringify({
          '差异对比': {
            '认证方式': '青龙: Client ID + Secret → 满天星: App Key + App Secret',
            'Token获取': '青龙: GET /open/auth/token → 满天星: POST /api/open-api/token',
            'Token有效期': '青龙: 不固定 → 满天星: 24 小时',
            '添加变量': '青龙: POST /open/envs (数组) → 满天星: POST /api/envs (对象)',
            '更新变量': '青龙: PUT /open/envs (body带id) → 满天星: PUT /api/envs/{id} (路径参数)',
            '删除变量': '青龙: DELETE /open/envs (数组) → 满天星: DELETE /api/envs/{id} (路径参数)',
            'Value编码': '青龙: 需要 URL 编码 → 满天星: 不需要编码',
          },
          '错误响应格式': { error: '错误信息' },
          'HTTP状态码': {
            '200': '成功',
    ...
    endpoints: [
      {
        ...
        path: '/api/scripts',
        title: '获取脚本列表',
        description: '获取所有脚本文件列表。所有 /api/scripts/* 接口（读取、保存、删除、改名、移动、复制、批量删除、回滚、调试运行等）的路径都相对脚本目录，并且一律拒绝下面这些目录，路径里任意一段命中都算、不区分大小写，返回 400 {"error":"该路径不可访问"}：.git、.svn、.hg、.bzr（版本库元数据，订阅用 Token 鉴权时 .git/config 里存着带访问令牌的地址，能读就等于泄露令牌，所以写死不可配置）；node_modules、__pycache__（依赖与字节码缓存，不属于脚本本身，脚本列表、统计与备份里也都忽略）。这是有意的安全限制，重试不会成功；确实要清理这些目录，可以用 POST /api/scripts/run-code 执行 shell 命令。',
        auth: 'jwt',
      ...
      },
      {
        id: 'scripts-delete',
        method: 'DELETE',
        path: '/api/scripts?path=:path&type=:type',
        title: '删除脚本',
        description: '删除脚本目录里的一个文件，或整个目录（type=directory，连同里面的全部内容），不可恢复。删非空目录必须传 type=directory，否则返回 500「删除文件失败」。.git、node_modules、__pycache__ 这类被隔离的目录不能删除，返回 400 {"error":"该路径不可访问"}（原因见「获取脚本列表」）。',
        auth: 'jwt',
        queryParams: [
          { name: 'path', type: 'string', required: true, description: '文件或目录的相对路径（相对脚本目录）', example: 'demo/old.py' },
          { name: 'type', type: 'string', description: 'file（默认）或 directory', example: 'file' },
        ],
        responseExample: JSON.stringify({ message: '删除成功' }, null, 2),
      },
      {
        id: 'scripts-run-code',
        method: 'POST',
        path: '/api/scripts/run-code',
        title: '执行代码片段',
        description: '不落盘直接执行一段代码，立即返回 run_id（HTTP 201），用「获取调试运行输出」轮询结果。language 区分大小写，可选 python、javascript、node（按 ES 模块 .mjs 执行）、mjs、typescript、shell、bash、go；bash 是 shell 的别名（v3.3.0 起），同样交给 bash 执行。工作目录是系统临时目录下的 starry-debug，不是脚本目录，访问脚本请用绝对路径，脚本目录的绝对路径在环境变量 STARRY_SCRIPTS_DIR 里。需要 scripts 权限，等于能在面板上执行任意代码。',
        auth: 'jwt',
        bodyParams: [
          { name: 'code', type: 'string', required: true, description: '要执行的代码', example: 'ls -la "$STARRY_SCRIPTS_DIR"' },
          { name: 'language', type: 'string', required: true, description: '语言：python / javascript / node / mjs / typescript / shell / bash / go（区分大小写）', example: 'bash' },
        ],
        responseExample: JSON.stringify({ message: '代码已启动', run_id: 'code_1758160000000' }, null, 2),
        responseFields: [
          { name: 'run_id', type: 'string', description: '本次运行的 ID，用于取输出、停止与清除' },
          { name: '400', type: 'tip', description: 'language 不在上面的列表里时返回 {"error":"不支持的语言类型"}' },
        ],
      },
      {
        id: 'scripts-run-logs',
        method: 'GET',
        path: '/api/scripts/run/:run_id/logs',
        title: '获取调试运行输出',
        description: '读取 run-code 或调试运行（POST /api/scripts/run）的输出。done 为 true 表示已结束，此时 exit_code 与 status 为最终结果；短命令通常第一次读取就已结束。未结束时可用 PUT /api/scripts/run/:run_id/stop 停止。运行记录只保存在面板内存里，取完结果后可调 DELETE /api/scripts/run/:run_id 清除。',
        auth: 'jwt',
        pathParams: [{ name: 'run_id', type: 'string', required: true, description: '执行代码片段 / 调试运行返回的 run_id' }],
        responseExample: JSON.stringify({ data: { logs: ['total 8', 'drwxr-xr-x 2 root root 4096 demo'], done: true, exit_code: 0, status: 'success' } }, null, 2),
        responseFields: [
          { name: 'data.logs', type: 'array', description: '输出行（标准输出与标准错误合在一起）' },
          { name: 'data.done', type: 'boolean', description: '是否已结束' },
          { name: 'data.exit_code', type: 'integer', description: '退出码，未结束时为 null' },
          { name: 'data.status', type: 'string', description: 'running / success / failed / stopped' },
        ],
      },
      {
        id: 'scripts-upload',
    ...
    endpoints: [
      ...
      },
      {
        ...
        responseExample: JSON.stringify({
          ...
          browsers_path: '/app/Starry-Panel/deps/ms-playwright',
        ...
        responseFields: [
          ...
          { name: 'browsers_path', type: 'string', description: 'Chromium 的下载目录，也就是任务里 PLAYWRIGHT_BROWSERS_PATH 的实际取值。优先级：环境变量页里启用的同名变量 > 面板进程的环境变量 > 默认值。Docker 部署的默认值是数据目录下的 deps/ms-playwright（默认 /app/Starry-Panel/deps/ms-playwright），在数据卷里，重建容器不丢；Windows、裸机与 Magisk 部署没有默认值' },
      ...
      },
      {
        ...
        responseExample: JSON.stringify({
          ...
          '不支持当前环境 (400)': {
            error: 'Alpine 镜像（musl）跑不了 Playwright 官方的 Chromium（glibc 构建），请换 Debian 版镜像 KKXM112/starry-panel:debian',
          },
        }, null, 2),
        responseFields: [
  ...
  },
  {
    ...
    endpoints: [
      ...
      },
      {
        ...
        description: '使用 App Key 和 App Secret 获取访问令牌，有效期 24 小时。⚠️ 开放 API（REST）的鉴权与 MCP 不同：REST 接口只认 Bearer 令牌，必须先调本接口换取 access_token（expires_in 为 86400 秒），再在请求头带 Authorization: Bearer <access_token>。把 app_secret 直接当 Bearer 令牌会返回 401 {"error":"令牌无效或已过期"}；REST 接口也不接受 Basic，带 Basic 或不带 Authorization 都返回 401 {"error":"缺少授权令牌"}。只有 MCP 接口（/api/v1/mcp）可以直接用 Authorization: Basic base64(app_key:app_secret)。令牌里带着应用勾选的权限范围，调用没勾选的模块返回 403 {"error":"应用无权访问此资源"}。',
        auth: 'none',
        bodyParams: [
  ...
  -d '{"jsonrpc":"2.0","id":1,"method":"tools/list","params":{}}'`,
          ...
          '客户端配置（stdio · Docker）': JSON.stringify({
            mcpServers: {
              'starry-panel': {
                command: 'docker',
                args: ['exec', '-i', 'starry-panel', 'ddp', 'mcp', '--app-key', '<app_key>', '--app-secret', '<app_secret>'],
              },
            },
          }, null, 2),
        },
        ...
        responseFields: [
          { name: 'result.tools', type: 'array', description: '当前凭据可用的工具（name / title / description / inputSchema / annotations）。只开启 MCP 时是 15 个查询类工具，打开「允许写入与执行工具」后是 44 个；示例只列出一个、省略了部分参数' },
      ...
      },
      {
        ...
        path: '/api/v1/mcp',
        title: '调用工具与工具清单',
        description: '用 tools/call 调用工具：params.name 是工具名，params.arguments 是参数（每个工具的参数说明见 tools/list 返回的 inputSchema）。共 44 个工具，下表按「只读 / 写入」列出，并注明各自需要应用勾选的权限范围：15 个查询类工具开启 MCP 服务即可用；29 个写入与执行类工具还要在「系统设置 → MCP 服务」打开「允许写入与执行工具」，否则 tools/list 里看不到，直接调用也会失败；其中会删除或覆盖已有数据的带 destructiveHint=true。应用没勾选对应权限、参数不对或面板接口报错时，返回 result.isError=true，文本里带原因。单次输出上限约 64KB，超出会截断并注明，列表类工具可减小 page_size 或加关键词。read_script 按字节分段读取：传 offset / limit（默认且最多 49152 字节），返回 total_bytes、next_offset 与 truncated，truncated 为 true 时用 next_offset 继续读，各段拼起来就是完整文件。list_envs、export_envs 会遮蔽名称像凭据的变量值（规则与环境变量页一致），按关键词搜索时也不会按这些变量的值匹配。脚本类工具与 /scripts/* 接口一样，不能操作 .git 等版本库元数据目录、node_modules 与 __pycache__（返回「该路径不可访问」）。',
        auth: 'jwt',
        extraHeaders: { Accept: 'application/json, text/event-stream' },
        bodyParams: [
          { name: 'jsonrpc', type: 'string', required: true, description: '固定为 "2.0"', example: '2.0' },
        ...
        responseFields: [
          ...
          { name: 'export_envs', type: '只读', description: '权限 envs。不分页导出环境变量（格式与 import_envs 一致），敏感变量同样遮蔽' },
          { name: 'list_scripts', type: '只读', description: '权限 scripts。脚本文件扁平列表' },
          { name: 'get_script_tree', type: '只读', description: '权限 scripts。脚本目录树，可按 path 只看子目录、按 max_depth 限制层数' },
          { name: 'read_script', type: '只读', description: '权限 scripts。按字节分段读取脚本内容（offset / limit），返回 total_bytes、next_offset、truncated' },
          { name: 'list_script_versions', type: '只读', description: '权限 scripts。脚本最近 50 个历史版本' },
          { name: 'list_subscriptions', type: '只读', description: '权限 subscriptions。查询订阅，含白名单、黑名单、依赖规则与最近拉取时间' },
          { name: 'list_backups', type: '只读', description: '权限 backup。备份目录里的备份文件' },
          { name: 'get_system_info', type: '只读', description: '权限 system。资源占用、面板版本、机器码、部署形态' },
          ...
          { name: 'create_task', type: '写入', description: '权限 tasks。新建任务（名称、命令、定时规则、类型、超时、标签等），建好即启用' },
          { name: 'update_task', type: '写入', description: '权限 tasks。按 ID 修改任务，只改传入的字段；脚本改名后可用它同步命令' },
          ...
          { name: 'batch_env_action', type: '写入', description: '权限 envs。批量 enable / disable / delete 环境变量' },
          { name: 'import_envs', type: '写入', description: '权限 envs。批量导入：merge（默认）覆盖名称与备注相同的变量；replace 先删除全部现有变量再导入' },
          { name: 'save_script', type: '写入', description: '权限 scripts。新建或覆盖脚本，保留历史版本，可用 rollback_script 回滚' },
          { name: 'run_script', type: '写入', description: '权限 scripts。调试运行脚本并等待最多约 50 秒；仍在运行时返回 run_id，用它再调用可继续取输出，加 stop: true 可停止' },
          { name: 'run_code', type: '写入', description: '权限 scripts。不落盘直接执行代码片段（shell / bash / python / javascript / node / typescript / go），等待与续取方式同 run_script' },
          { name: 'delete_script', type: '写入', description: '权限 scripts。删除文件，或 type=directory 删除整个目录' },
          { name: 'batch_delete_scripts', type: '写入', description: '权限 scripts。一次删除最多 100 个文件或目录，返回失败项' },
          { name: 'rename_script / move_script / copy_script', type: '写入', description: '权限 scripts。重命名、移动、复制文件或目录，目标已有同名文件会被覆盖' },
          { name: 'rollback_script', type: '写入', description: '权限 scripts。把脚本恢复到某个历史版本' },
          { name: 'pull_subscription', type: '写入', description: '权限 subscriptions。立即拉取订阅，在后台进行' },
          { name: 'create_subscription / update_subscription', type: '写入', description: '权限 subscriptions。新建、修改订阅（修改只改传入的字段）' },
          { name: 'delete_subscription', type: '写入', description: '权限 subscriptions。删除订阅及其拉取记录，不删已同步的脚本与任务' },
          { name: 'set_subscription_enabled', type: '写入', description: '权限 subscriptions。启用或禁用订阅' },
          { name: 'send_notification', type: '写入', description: '权限 notifications。通过已配置的渠道发通知，可按渠道 ID / 名称 / 类型指定' },
          { name: 'create_backup / delete_backup', type: '写入', description: '权限 backup。创建（可只备份部分内容、可加密）或删除备份文件' },
          { name: 'restore_backup', type: '写入', description: '权限 backup。用备份覆盖当前数据：备份包含的每一项都会先清空再写入，恢复配置还会替换开放 API 应用；恢复后请重启面板' },
          { name: 'scripts / tasks 权限', type: 'tip', description: 'run_code、保存脚本再运行、新建或修改任务命令，都等于能在面板上执行任意代码；envs 权限能改写所有凭据；backup 权限能用备份覆盖整个面板。请只授予完全信任的客户端' },
        ],
      },
    ],
  },
    ...
    endpoints: [
      {
        ...
        title: '获取系统信息',
        description: '获取服务器系统信息（CPU/内存/磁盘/机器码等）。自 v3.0.4 起同层追加 deployment_type（docker / binary / magisk）与 magisk_shell_version（Magisk 模块外壳版本，非模块版为 0）；自 v3.3.0 起再追加 version（面板版本号），原有字段位置不变。多实例巡检时，拿一次本接口就能用 hostname + machine_code + version 对齐各实例的数据，不必再单独请求 /api/system/version。',
        auth: 'jwt',
        responseExample: JSON.stringify({
          data: { hostname: 'server', machine_code: 'A1B2C3D4-E5F6-4789-ABCD-1234567890AB', os: 'linux', arch: 'amd64', go_version: 'go1.22.0', num_cpu: 4, cpu_usage: 25.0, memory_total: 8589934592, memory_used: 4294967296, memory_usage: 50.0, disk_total: 107374182400, disk_used: 53687091200, disk_usage: 50.0, deployment_type: 'docker', magisk_shell_version: 0, version: '3.3.0' },
        }, null, 2),
        responseFields: [
          { name: 'data.machine_code', type: 'string', description: '面板机器码（大写 UUID v4），首次启动时生成并存进数据库，重启、升级都不会变，可作实例唯一标识。注意：用另一个实例的备份恢复「配置」后，机器码会变成那个实例的' },
          { name: 'data.version', type: 'string', description: '面板版本号（v3.3.0 起），与 /api/system/version 返回的 version 相同' },
          { name: 'data.hostname', type: 'string', description: '主机名。容器部署时通常是容器 ID，重建容器会变，不适合当实例标识' },
          { name: 'data.deployment_type', type: 'string', description: '部署形态：docker / binary / magisk' },
        ],
      },
      {
        ...
        title: '获取面板机器码',
        description: '直接读取面板首次启动生成的机器码（UUID v4 大写格式）。不再需要手工查 SQL，OpenAPI 应用具备 system 权限即可调用。机器码重启、升级都不会变，可作实例唯一标识；/api/system/info 里也带着同一个 machine_code。',
        auth: 'jwt',
    ...
    endpoints: [
      {
        ...
        responseExample: JSON.stringify({ data: { path: 'data/backups/starry-backup-20260101-120000.json' }, message: '备份成功' }, null, 2),
      },
      {
        id: 'backup-list',
        method: 'GET',
        ...
        auth: 'jwt',
        responseExample: JSON.stringify({ data: [{ filename: 'starry-backup-20260101.json', size: 102400, modified_at: '2026-01-01T12:00:00Z' }] }, null, 2),
      },
      {
        id: 'backup-delete',
        method: 'DELETE',
