 ...
 *
 * 「需要减少动效」要综合个人设置页的「界面动效」三档（C8），口径见 utils/panelAppearance.ts 的 shouldReduceMotion()。
 */
        ...
        <template v-else>
          <div class="login-header">
            <div class="login-logo">
              <img src="/favicon-512.webp" alt="满天星面板" width="48" height="48" />
            </div>
            <h2>{{ titleText }}</h2>
            <p>{{ subtitleText }}</p>
          </div>
          ...
          <div class="login-version">
            满天星面板{{ panelVersion ? ` v${panelVersion}` : "" }}
          </div>
        </template>
      </div>
    </div>
