    ...
    transformIndexHtml: {
      ...
      handler(html) {
        ...
        if (!metaRe.test(html)) {
          ...
          throw new Error(
            '[starry-robots-meta] 在 index.html 中找不到 name="robots" 的 meta。\n' +
  ...
  return {
    ...
    buildStart() {
      ...
      if (!fs.existsSync(cssFile)) {
        throw new Error('[starry-selfhosted-fonts] 缺少 web/public/fonts/fonts.css')
      }
      ...
      if (missing.length > 0) {
        throw new Error(
          `[starry-selfhosted-fonts] fonts.css 引用的 ${missing.length} 个 woff2 不在磁盘上：\n` +
            `  ${missing.join('\n  ')}\n` +
